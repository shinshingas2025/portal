...
Imports System.IO
Namespace EIIS
    Public Class EditSecurityAuthority
...
#End Region
        ...
        Private Function GetTempTreeFile() As String
            Dim fn As String
            fn = "A" & sid & "_" & Year(Now()) & Right("0" & Month(Now()), 2) & Right("0" & Day(Now()), 2) & Right("0" & Hour(Now()), 2) & Right("0" & Minute(Now()), 2) & Right("0" & Second(Now()), 2) & ".xml"
            Return fn
        End Function
        ...
        Private Sub ShowTree(ByVal treeFile As String)
            ...
            Dim fls() As String = Directory.GetFiles(Server.MapPath("/PortalFiles/xml/SecAut"), "A" & sid & "_" & "*.xml")

            If UBound(fls) < 1 Then
                WriteXmlTree()
                fls = Directory.GetFiles(Server.MapPath("/Portalfiles/xml/SecAut"), "A" & sid & "_" & "*.xml")
            End If
        ...
        Private Sub WriteXmlTree()
            ...
            Do
                ...
                If icount = 1 Then
                    strSQL = "Select * from sysCommunity where  PID in (" & levelNode & ")  and objid=" & rootnum1 & "  order by seqno "
                Else
            ...
            xmlDoc.Save(Server.MapPath("/PortalFiles/xml/SecAut/" & TreefileTemp))
        ...
        Private Sub QueryNode(ByVal nd As String)
            ...
            'node = TreeView1.Nodes(nd.Split(CType(" ", Char))(0))
        ...
        Private Sub btnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDelete.Click
            ...
            If TreeView1.SelectedValue = "0" Then
                errmsg.Text = "該項目無法刪除!"
                Exit Sub
            End If
        ...
        Private Sub TreeView1_SelectedNodeChanged(ByVal sender As System.Object, ByVal e As EventArgs) Handles TreeView1.SelectedNodeChanged
        ...
        Private Sub addfunction_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles addfunction.Click
            txtState.Text = CType(ShowDialogBox(txtState, "ImportDomains.aspx?UID=" & txtobjID.Text.Trim, 400, 520, 0, 0, True), String)
        End Sub
        ...
        Private Sub DataGrid3_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DataGrid3.SelectedIndexChanged
            ...
            UserID = datagriditem.Cells(0).Text.Trim
        ...
        Private Sub DataGrid1_UpdateCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles DataGrid1.UpdateCommand
            ...
            '(2) 找對應的實體類別，在se.UpdateaAccount(at)的se按照步驟去抓定義，最後抓到是[portal]->[sysSecurity]Table:StartDate和EndDate都是DateTime格式
            ...
            '---115/1/22以前的寫法，Ctype強制轉型會導致型別轉換錯誤' 會把UI頁面的"下午"傳到SQL做DateTime轉換時，因為格式不符而錯誤
            'at.StartDate = CType(txtStartDate.Text.Trim, Date)
            'at.EndDate = CType(txtEndDate.Text.Trim, Date)

            ' 115/1/22 修正如下: 先轉成日期物件，再確保存回物件時是正確的-->再去改SecurityBO.vb的sql語法
            ' 先宣告兩個變數來存轉換結果
            Dim startDateValue As Date
            Dim endDateValue As Date
            ' 處理 StartDate：嘗試解析字串，失敗的話給一個預設值（例如今天）
            If Date.TryParse(txtStartDate.Text.Trim, startDateValue) Then
                at.StartDate = startDateValue
            Else
                ' 如果解析失敗（例如格子是空的或是格式亂掉），可以設定為今天
                at.StartDate = DateTime.Now
            End If
            ' 處理 EndDate
            If Date.TryParse(txtEndDate.Text.Trim, endDateValue) Then
                at.EndDate = endDateValue
            Else
                ' 如果解析失敗，通常有效日期迄會給一個遙遠的未來（例如 2099 年）
                at.EndDate = New Date(2099, 12, 31)
            End If
            ' 115/1/22 修正結束'
            ...
            '(1) 115/1/22錯誤訊息:日期型別轉換，找上面有沒有Account相關Dim宣告'
        ...
        Private Sub DataGrid4_PageIndexChanged1(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridPageChangedEventArgs) Handles DataGrid4.PageIndexChanged
            DataGrid4.CurrentPageIndex = e.NewPageIndex
            Dim ur As New Domains

            Dim dt As New DataTable
            Dim st As String
            st = CType(txtobjID.Text, Integer)

            dt = ur.GetMapDataByCommunityObjID(st)
            DataGrid4.DataSource = dt
            DataGrid4.DataBind()
