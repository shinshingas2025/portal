...
#End Region
    ...
    Sub showData()
        ...
        Dim sdata_ym As String = ""
        Dim saction As String = "9"
        Dim slikeSelect As String = ""
        Dim slikeContent As String = ""

        msgbox.Text = ""
        sDateStart = getDate(txtDateStart.Text.ToString.Trim, "1911/01/01")
        sDateEnd = getDate(txtDateEnd.Text.ToString.Trim, "2999/12/31")
        sdata_ym = txtdata_ym.Text.ToString.Trim
        saction = txtaction.SelectedValue.ToString.Trim
        slikeSelect = likeSelect.SelectedValue.ToString.Trim
        slikeContent = likeContent.Text.ToString.Trim

        If msgbox.Text <> "" Then
            Exit Sub
        End If

        objCartDT = se.UserQuery(sDateStart, sDateEnd, sdata_ym, saction, slikeSelect, slikeContent)
    ...
    Private Sub dgCart_ItemCommand1(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles dgCart.ItemCommand
        ...
        '取得選取的資料ID
        If i <> -1 Then
            swm_no = CType(dgCart.DataKeys(i), Integer)
            suser_id = CType(dgCart.Items(i).Cells(0).Text, String)
            suser_name = CType(dgCart.Items(i).Cells(1).Text, String)
            shouse_no = CType(dgCart.Items(i).Cells(2).Text, String)
            Response.Redirect("ers_send_detailMgt.aspx?wm_no=" & swm_no & "&user_id=" & suser_id & "&user_name=" & suser_name & "&house_no=" & shouse_no)
        End If
