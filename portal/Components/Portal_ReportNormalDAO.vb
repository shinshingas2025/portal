...
Namespace ASPNET.StarterKit.Portal
	Public Class Portal_ReportNormalDAO
		Public Overridable Function GetEntity(ByVal reportGroupID As String, ByVal itemID As Integer) As DataSet
			Dim myConnection As New SqlConnection(ConfigurationSettings.AppSettings("connectionString"))
			Dim myCommand As New SqlCommand("select * from Portal_ReportNormal where ReportGroupID=@ReportGroupID and ItemID=@ItemID order by EntityID desc", myConnection)
		...
		Public Overridable Function GetEntitys(ByVal reportGroupID As String) As DataSet
			Dim myConnection As New SqlConnection(ConfigurationSettings.AppSettings("connectionString"))
			Dim myCommand As New SqlCommand("select * from Portal_ReportNormal where ReportGroupID=@ReportGroupID order by EntityID desc", myConnection)
			myCommand.Parameters.Add("@ReportGroupID", SqlDbType.NVarChar, 16).Value = reportGroupID
			Dim myAdapter As New SqlDataAdapter(myCommand)
			Dim myDataSet As New DataSet
			myAdapter.Fill(myDataSet)
		...
		Public Overridable Sub DeleteEntity(ByVal entityID As String)
			Dim myConnection As New SqlConnection(ConfigurationSettings.AppSettings("connectionString"))
			Dim myCommand As New SqlCommand("delete from Portal_ReportNormal where EntityID=@EntityID", myConnection)
			myCommand.Parameters.Add("@EntityID", SqlDbType.NVarChar, 24).Value = entityID
			myConnection.Open()
		...
		Public Overridable Function InsertEntity(ByVal reportGroupID As String, ByVal itemID As Integer, ByVal name As String, ByVal state As Integer, ByVal displayOrder As Integer, ByVal reportName As String, ByVal procedureName As String, ByVal createdByUser As String, ByVal createdDate As Date) As String
			...
			Dim myConnection As New SqlConnection(ConfigurationSettings.AppSettings("connectionString"))
			Dim myCommand As New SqlCommand("insert into Portal_ReportNormal ( EntityID,ReportGroupID,ItemID,Name,State,DisplayOrder,ReportName,ProcedureName,CreatedByUser,CreatedDate ) values ( @EntityID,@ReportGroupID,@ItemID,@Name,@State,@DisplayOrder,@ReportName,@ProcedureName,@CreatedByUser,@CreatedDate )", myConnection)
		...
		Public Overridable Function GetMaxEntityID(ByVal entityID As String) As String
			...
			selectSQL = "select max(EntityID) from Portal_ReportNormal where substring(EntityID,1,16)='" & entityID.substring(0, 16) & "'"
		...
		Public Overridable Function GetTotalRow() As Integer
			Dim valResult As Integer
			Dim myConnection As New SqlConnection(ConfigurationSettings.AppSettings("connectionString"))
			Dim myCommand As New SqlCommand("select count(*) from Portal_ReportNormal", myConnection)
			myConnection.Open()
			Dim myReader As SqlDataReader
			myReader = myCommand.ExecuteReader(CommandBehavior.CloseConnection)
			While myReader.Read()
