...
Namespace ASPNET.StarterKit.Portal.AuditSystem.DAO
	Public Class ResolutionAuditCodeDAO
		Public Overridable Function GetEntity(ByVal itemID As Integer) As DataSet
			...
			Dim myCommand As New SqlCommand("select * from ResolutionAuditCode where ItemID=@ItemID order by EntityID desc", myConnection)
		...
		Public Overridable Sub DeleteEntity(ByVal entityID As String)
			Dim myConnection As New SqlConnection(ConfigurationSettings.AppSettings("AuditSystemConnectionString"))
			Dim myCommand As New SqlCommand("delete from ResolutionAuditCode where EntityID=@EntityID",myConnection)
			myCommand.Parameters.Add("@EntityID", SqlDbType.NVarChar,16).Value=entityID
			myConnection.Open()
		...
		Public Overridable Function InsertEntity(ByVal itemID As Integer,ByVal typeID As Integer,ByVal name As String,ByVal description As String,ByVal displayOrder As Integer,ByVal creatorID As String,ByVal createdDate As Date,ByVal modifierID As String,ByVal modifiedDate As Date,ByVal permission As String,ByVal permissionGroup As String,ByVal state As Integer,ByVal deletedDate As Date) As String
			...
			Dim myConnection As New SqlConnection(ConfigurationSettings.AppSettings("AuditSystemConnectionString"))
			Dim myCommand As New SqlCommand("insert into ResolutionAuditCode ( EntityID,ItemID,TypeID,Name,Description,DisplayOrder,CreatorID,CreatedDate,ModifierID,ModifiedDate,Permission,PermissionGroup,State,DeletedDate ) values ( @EntityID,@ItemID,@TypeID,@Name,@Description,@DisplayOrder,@CreatorID,@CreatedDate,@ModifierID,@ModifiedDate,@Permission,@PermissionGroup,@State,@DeletedDate )", myConnection)
		...
		Public Overridable Function GetMaxEntityID(ByVal entityID As String) As String
			...
			selectSQL="select max(EntityID) from ResolutionAuditCode where substring(EntityID,1,8)='" & entityID.substring(0,8) & "'"
		...
		Public Overridable Function GetTotalRow() As Integer
			Dim valResult As Integer
			Dim myConnection As New SqlConnection(ConfigurationSettings.AppSettings("AuditSystemConnectionString"))
			Dim myCommand As New SqlCommand("select count(*) from ResolutionAuditCode", myConnection)
			myConnection.Open()
			Dim myReader As SqlDataReader
			myReader = myCommand.ExecuteReader(CommandBehavior.CloseConnection)
			While myReader.Read()
