...
Namespace ASPNET.StarterKit.Portal.AuditSystem.DAO
	Public Class ScheduleDAO
		Public Overridable Function GetEntity(ByVal itemID As Integer) As DataSet
			...
			Dim myCommand As New SqlCommand("select * from Schedule where ItemID=@ItemID order by EntityID desc", myConnection)
		...
		Public Overridable Sub DeleteEntity(ByVal entityID As String)
			Dim myConnection As New SqlConnection(ConfigurationSettings.AppSettings("AuditSystemConnectionString"))
			Dim myCommand As New SqlCommand("delete from Schedule where EntityID=@EntityID", myConnection)
			myCommand.Parameters.Add("@EntityID", SqlDbType.NVarChar, 16).Value = entityID
			myConnection.Open()
			myCommand.ExecuteNonQuery()
			myConnection.Close()
		End Sub
		Public Overridable Function InsertEntity(ByVal itemID As Integer, ByVal scheduleTypeID As Integer, ByVal eventTypeID As Integer, ByVal userID As Integer, ByVal startDate As Date, ByVal endDate As Date, ByVal levelID As Integer, ByVal title As String, ByVal description As String, ByVal note As String, ByVal keyWord As String, ByVal creatorID As String, ByVal createdDate As Date, ByVal modifierID As String, ByVal modifiedDate As Date, ByVal permission As String, ByVal permissionGroup As String, ByVal state As Integer, ByVal deletedDate As Date) As String
			Dim entityID As String
			Dim myConnection As New SqlConnection(ConfigurationSettings.AppSettings("AuditSystemConnectionString"))
			Dim myCommand As New SqlCommand("insert into Schedule ( EntityID,ItemID,ScheduleTypeID,EventTypeID,UserID,StartDate,EndDate,LevelID,Title,Description,Note,KeyWord,CreatorID,CreatedDate,ModifierID,ModifiedDate,Permission,PermissionGroup,State,DeletedDate ) values ( @EntityID,@ItemID,@ScheduleTypeID,@EventTypeID,@UserID,@StartDate,@EndDate,@LevelID,@Title,@Description,@Note,@KeyWord,@CreatorID,@CreatedDate,@ModifierID,@ModifiedDate,@Permission,@PermissionGroup,@State,@DeletedDate )", myConnection)
			...
			Return entityID
		End Function
		Public Overridable Sub UpdateEntity(ByVal entityID As String, ByVal scheduleTypeID As Integer, ByVal eventTypeID As Integer, ByVal userID As Integer, ByVal startDate As Date, ByVal endDate As Date, ByVal levelID As Integer, ByVal title As String, ByVal description As String, ByVal note As String, ByVal keyWord As String, ByVal creatorID As String, ByVal createdDate As Date, ByVal modifierID As String, ByVal modifiedDate As Date, ByVal permission As String, ByVal permissionGroup As String, ByVal state As Integer, ByVal deletedDate As Date)
			Dim myConnection As New SqlConnection(ConfigurationSettings.AppSettings("AuditSystemConnectionString"))
			Dim myCommand As New SqlCommand("update Schedule set ScheduleTypeID=@ScheduleTypeID , EventTypeID=@EventTypeID , UserID=@UserID , StartDate=@StartDate , EndDate=@EndDate , LevelID=@LevelID , Title=@Title , Description=@Description , Note=@Note , KeyWord=@KeyWord , CreatorID=@CreatorID , CreatedDate=@CreatedDate , ModifierID=@ModifierID , ModifiedDate=@ModifiedDate , Permission=@Permission , PermissionGroup=@PermissionGroup , State=@State , DeletedDate=@DeletedDate where EntityID=@EntityID", myConnection)
			myCommand.Parameters.Add("@ScheduleTypeID", SqlDbType.Int, 4).Value = scheduleTypeID
			myCommand.Parameters.Add("@EventTypeID", SqlDbType.Int, 4).Value = eventTypeID
			myCommand.Parameters.Add("@UserID", SqlDbType.Int, 4).Value = userID
			myCommand.Parameters.Add("@StartDate", SqlDbType.DateTime, 8).Value = startDate
		...
		Public Overridable Function GetMaxEntityID(ByVal entityID As String) As String
			...
			selectSQL = "select max(EntityID) from Schedule where substring(EntityID,1,8)='" & entityID.substring(0, 8) & "'"
		...
		Public Overridable Function GetTotalRow() As Integer
			Dim valResult As Integer
			Dim myConnection As New SqlConnection(ConfigurationSettings.AppSettings("AuditSystemConnectionString"))
			Dim myCommand As New SqlCommand("select count(*) from Schedule", myConnection)
			myConnection.Open()
			Dim myReader As SqlDataReader
			myReader = myCommand.ExecuteReader(CommandBehavior.CloseConnection)
			While myReader.Read()
