...
Namespace ASPNET.StarterKit.Portal.AuditSystem.DAO
	Public Class NewsReleaseAppendDAO
		Public Overridable Function GetEntity(ByVal newsReleaseID As String,ByVal itemID As Integer) As DataSet
			...
			Dim myCommand As New SqlCommand("select * from NewsReleaseAppend where NewsReleaseID=@NewsReleaseID and ItemID=@ItemID order by EntityID desc", myConnection)
		...
		Public Overridable Sub DeleteEntity(ByVal entityID As String)
			Dim myConnection As New SqlConnection(ConfigurationSettings.AppSettings("AuditSystemConnectionString"))
			Dim myCommand As New SqlCommand("delete from NewsReleaseAppend where EntityID=@EntityID",myConnection)
			myCommand.Parameters.Add("@EntityID", SqlDbType.NVarChar, 40).Value = entityID
			myConnection.Open()
			myCommand.ExecuteNonQuery()
			myConnection.Close()
		End Sub
		Public Overridable Function InsertEntity(ByVal newsReleaseID As String,ByVal itemID As Integer,ByVal name As String,ByVal description As String,ByVal fileName As String,ByVal fileSize As Integer,ByVal displayOrder As Integer,ByVal creatorID As String,ByVal createdDate As Date,ByVal modifierID As String,ByVal modifiedDate As Date,ByVal permission As String,ByVal permissionGroup As String,ByVal state As Integer,ByVal deletedDate As Date) As String
			Dim entityID As String
			Dim myConnection As New SqlConnection(ConfigurationSettings.AppSettings("AuditSystemConnectionString"))
			Dim myCommand As New SqlCommand("insert into NewsReleaseAppend ( EntityID,NewsReleaseID,ItemID,Name,Description,FileName,FileSize,DisplayOrder,CreatorID,CreatedDate,ModifierID,ModifiedDate,Permission,PermissionGroup,State,DeletedDate ) values ( @EntityID,@NewsReleaseID,@ItemID,@Name,@Description,@FileName,@FileSize,@DisplayOrder,@CreatorID,@CreatedDate,@ModifierID,@ModifiedDate,@Permission,@PermissionGroup,@State,@DeletedDate )", myConnection)
		...
		Public Overridable Function GetMaxEntityID(ByVal entityID As String) As String
			...
			selectSQL = "select max(EntityID) from NewsReleaseAppend where substring(EntityID,1,32)='" & entityID.Substring(0, 32) & "'"
		...
		Public Overridable Function GetTotalRow() As Integer
			Dim valResult As Integer
			Dim myConnection As New SqlConnection(ConfigurationSettings.AppSettings("AuditSystemConnectionString"))
			Dim myCommand As New SqlCommand("select count(*) from NewsReleaseAppend", myConnection)
			myConnection.Open()
			Dim myReader As SqlDataReader
			myReader = myCommand.ExecuteReader(CommandBehavior.CloseConnection)
			While myReader.Read()
