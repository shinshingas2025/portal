...
Namespace ASPNET.StarterKit.Portal.AuditSystem.DAO
	Public Class NewsReleaseDAO
		Public Overridable Function GetEntity(ByVal categorizationID As String,ByVal itemID As Integer) As DataSet
			Dim myConnection As New SqlConnection(ConfigurationSettings.AppSettings("AuditSystemConnectionString"))
			Dim myCommand As New SqlCommand("select * from NewsRelease where CategorizationID=@CategorizationID and ItemID=@ItemID order by EntityID desc", myConnection)
			...
			Return myDataSet
		End Function
		Public Overridable Function GetEntitys(ByVal categorizationID As String,ByVal itemID As Integer) As DataSet
			Dim myConnection As New SqlConnection(ConfigurationSettings.AppSettings("AuditSystemConnectionString"))
			Dim myCommand As New SqlCommand("select * from NewsRelease where CategorizationID=@CategorizationID and ItemID=@ItemID order by EntityID desc", myConnection)
			myCommand.Parameters.Add("@CategorizationID", SqlDbType.NVarChar,24).Value=categorizationID
			myCommand.Parameters.Add("@ItemID", SqlDbType.Int,4).Value=itemID
		...
		Public Overridable Sub DeleteEntity(ByVal entityID As String)
			Dim myConnection As New SqlConnection(ConfigurationSettings.AppSettings("AuditSystemConnectionString"))
			Dim myCommand As New SqlCommand("delete from NewsRelease where EntityID=@EntityID",myConnection)
			myCommand.Parameters.Add("@EntityID", SqlDbType.NVarChar, 32).Value = entityID
			myConnection.Open()
			myCommand.ExecuteNonQuery()
			myConnection.Close()
		End Sub
		Public Overridable Function InsertEntity(ByVal categorizationID As String, ByVal itemID As Integer, ByVal title As String, ByVal description As String, ByVal opening As String, ByVal ending As String, ByVal relationURL As String, ByVal releaseUnitID As String, ByVal liaisonerID As String, ByVal newsDate As Date, ByVal startDate As Date, ByVal endDate As Date, ByVal creatorID As String, ByVal createdDate As Date, ByVal modifierID As String, ByVal modifiedDate As Date, ByVal permission As String, ByVal permissionGroup As String, ByVal state As Integer, ByVal deletedDate As Date) As String
			Dim entityID As String
			Dim myConnection As New SqlConnection(ConfigurationSettings.AppSettings("AuditSystemConnectionString"))
			Dim myCommand As New SqlCommand("insert into NewsRelease ( EntityID,CategorizationID,ItemID,Title,Description,Opening,Ending,RelationURL,ReleaseUnitID,LiaisonerID,NewsDate,StartDate,EndDate,CreatorID,CreatedDate,ModifierID,ModifiedDate,Permission,PermissionGroup,State,DeletedDate ) values ( @EntityID,@CategorizationID,@ItemID,@Title,@Description,@Opening,@Ending,@RelationURL,@ReleaseUnitID,@LiaisonerID,@NewsDate,@StartDate,@EndDate,@CreatorID,@CreatedDate,@ModifierID,@ModifiedDate,@Permission,@PermissionGroup,@State,@DeletedDate )", myConnection)
			...
			Return entityID
		End Function
		Public Overridable Sub UpdateEntity(ByVal entityID As String, ByVal categorizationID As String, ByVal title As String, ByVal description As String, ByVal opening As String, ByVal ending As String, ByVal relationURL As String, ByVal releaseUnitID As String, ByVal liaisonerID As String, ByVal newsDate As Date, ByVal startDate As Date, ByVal endDate As Date, ByVal creatorID As String, ByVal createdDate As Date, ByVal modifierID As String, ByVal modifiedDate As Date, ByVal permission As String, ByVal permissionGroup As String, ByVal state As Integer, ByVal deletedDate As Date)
			Dim myConnection As New SqlConnection(ConfigurationSettings.AppSettings("AuditSystemConnectionString"))
			Dim myCommand As New SqlCommand("update NewsRelease set CategorizationID=@CategorizationID , Title=@Title , Description=@Description , Opening=@Opening , Ending=@Ending , RelationURL=@RelationURL , ReleaseUnitID=@ReleaseUnitID , LiaisonerID=@LiaisonerID , NewsDate=@NewsDate , StartDate=@StartDate , EndDate=@EndDate , CreatorID=@CreatorID , CreatedDate=@CreatedDate , ModifierID=@ModifierID , ModifiedDate=@ModifiedDate , Permission=@Permission , PermissionGroup=@PermissionGroup , State=@State , DeletedDate=@DeletedDate where EntityID=@EntityID", myConnection)
			myCommand.Parameters.Add("@CategorizationID", SqlDbType.NVarChar, 24).Value = categorizationID
			myCommand.Parameters.Add("@Title", SqlDbType.NVarChar, 128).Value = title
			myCommand.Parameters.Add("@Description", SqlDbType.NVarChar, 256).Value = description
			myCommand.Parameters.Add("@Opening", SqlDbType.NVarChar, 1024).Value = opening
		...
		Public Overridable Function GetMaxEntityID(ByVal entityID As String) As String
			...
			selectSQL = "select max(EntityID) from NewsRelease where substring(EntityID,1,24)='" & entityID.Substring(0, 24) & "'"
		...
		Public Overridable Function GetTotalRow() As Integer
			Dim valResult As Integer
			Dim myConnection As New SqlConnection(ConfigurationSettings.AppSettings("AuditSystemConnectionString"))
			Dim myCommand As New SqlCommand("select count(*) from NewsRelease", myConnection)
			myConnection.Open()
			Dim myReader As SqlDataReader
			myReader = myCommand.ExecuteReader(CommandBehavior.CloseConnection)
			While myReader.Read()
