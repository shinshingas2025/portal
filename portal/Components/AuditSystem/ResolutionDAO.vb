...
Namespace ASPNET.StarterKit.Portal.AuditSystem.DAO
	Public Class ResolutionDAO
		Public Overridable Function GetEntity(ByVal itemID As Integer) As DataSet
			...
			Dim myCommand As New SqlCommand("select * from Resolution where ItemID=@ItemID order by EntityID desc", myConnection)
		...
		Public Overridable Sub DeleteEntity(ByVal entityID As String)
			...
			myConnection.Close()
		End Sub
		Public Overridable Function InsertEntity(ByVal itemID As Integer, ByVal resolutionNumber As String, ByVal content As String, ByVal mainUnit As String, ByVal auditStateID As String, ByVal forecastDate As Date, ByVal finishID As Integer, ByVal note As String, ByVal permission As String, ByVal permissionGroup As String, ByVal creatorID As String, ByVal createdDate As Date, ByVal modifierID As String, ByVal modifiedDate As Date, ByVal state As Integer, ByVal deletedDate As Date) As String
			Dim entityID As String
			Dim myConnection As New SqlConnection(ConfigurationSettings.AppSettings("AuditSystemConnectionString"))
			Dim myCommand As New SqlCommand("insert into Resolution ( EntityID,ItemID,ResolutionNumber,Content,MainUnit,AuditStateID,ForecastDate,FinishID,Note,Permission,PermissionGroup,CreatorID,CreatedDate,ModifierID,ModifiedDate,State,DeletedDate ) values ( @EntityID,@ItemID,@ResolutionNumber,@Content,@MainUnit,@AuditStateID,@ForecastDate,@FinishID,@Note,@Permission,@PermissionGroup,@CreatorID,@CreatedDate,@ModifierID,@ModifiedDate,@State,@DeletedDate )", myConnection)
			...
			Return entityID
		End Function
		Public Overridable Sub UpdateEntity(ByVal entityID As String, ByVal resolutionNumber As String, ByVal content As String, ByVal mainUnit As String, ByVal auditStateID As String, ByVal forecastDate As Date, ByVal finishID As Integer, ByVal note As String, ByVal permission As String, ByVal permissionGroup As String, ByVal creatorID As String, ByVal createdDate As Date, ByVal modifierID As String, ByVal modifiedDate As Date, ByVal state As Integer, ByVal deletedDate As Date)
			Dim myConnection As New SqlConnection(ConfigurationSettings.AppSettings("AuditSystemConnectionString"))
			Dim myCommand As New SqlCommand("update Resolution set ResolutionNumber=@ResolutionNumber , Content=@Content , MainUnit=@MainUnit , AuditStateID=@AuditStateID , ForecastDate=@ForecastDate , FinishID=@FinishID , Note=@Note , Permission=@Permission , PermissionGroup=@PermissionGroup , CreatorID=@CreatorID , CreatedDate=@CreatedDate , ModifierID=@ModifierID , ModifiedDate=@ModifiedDate , State=@State , DeletedDate=@DeletedDate where EntityID=@EntityID", myConnection)
			myCommand.Parameters.Add("@ResolutionNumber", SqlDbType.NVarChar, 16).Value = resolutionNumber
			myCommand.Parameters.Add("@Content", SqlDbType.NVarChar, 1024).Value = content
			myCommand.Parameters.Add("@MainUnit", SqlDbType.NVarChar, 256).Value = mainUnit
			myCommand.Parameters.Add("@AuditStateID", SqlDbType.NVarChar, 24).Value = auditStateID
		...
		Public Overridable Function GetMaxEntityID(ByVal entityID As String) As String
			...
			selectSQL = "select max(EntityID) from Resolution where substring(EntityID,1,8)='" & entityID.Substring(0, 8) & "'"
		...
		Public Overridable Function GetTotalRow() As Integer
			Dim valResult As Integer
			Dim myConnection As New SqlConnection(ConfigurationSettings.AppSettings("AuditSystemConnectionString"))
			Dim myCommand As New SqlCommand("select count(*) from Resolution", myConnection)
			myConnection.Open()
			Dim myReader As SqlDataReader
			myReader = myCommand.ExecuteReader(CommandBehavior.CloseConnection)
			While myReader.Read()
