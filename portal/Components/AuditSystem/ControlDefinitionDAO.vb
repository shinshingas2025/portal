...
Namespace ASPNET.StarterKit.Portal.AuditSystem.DAO
	Public Class ControlDefinitionDAO
		Public Overridable Function GetEntity(ByVal itemID As Integer) As DataSet
			...
			Dim myCommand As New SqlCommand("select * from ControlDefinition where ItemID=@ItemID order by EntityID desc", myConnection)
		...
		Public Overridable Function GetSingleEntity(ByVal entityID As String) As SqlDataReader
			Dim myConnection As New SqlConnection(ConfigurationSettings.AppSettings("AuditSystemConnectionString"))
			Dim myCommand As New SqlCommand("select * from ControlDefinition where EntityID=@EntityID", myConnection)
		...
		Public Overridable Sub DeleteEntity(ByVal entityID As String)
			Dim myConnection As New SqlConnection(ConfigurationSettings.AppSettings("AuditSystemConnectionString"))
			Dim myCommand As New SqlCommand("delete from ControlDefinition where EntityID=@EntityID", myConnection)
			myCommand.Parameters.Add("@EntityID", SqlDbType.NVarChar, 16).Value = entityID
			myConnection.Open()
			myCommand.ExecuteNonQuery()
			myConnection.Close()
		End Sub
		Public Overridable Function InsertEntity(ByVal itemID As Integer, ByVal parentID As String, ByVal typeID As Integer, ByVal title As String, ByVal description As String, ByVal mobileSourceFile As String, ByVal desktopSourceFile As String, ByVal rowPosition As Integer, ByVal columnPosition As Integer, ByVal rowSpan As Integer, ByVal columnSpan As Integer, ByVal creatorID As String, ByVal createdDate As Date, ByVal modifierID As String, ByVal modifiedDate As Date, ByVal permission As String, ByVal permissionGroup As String, ByVal state As Integer, ByVal deletedDate As Date) As String
			Dim entityID As String
			Dim myConnection As New SqlConnection(ConfigurationSettings.AppSettings("AuditSystemConnectionString"))
			Dim myCommand As New SqlCommand("insert into ControlDefinition ( EntityID,ItemID,ParentID,TypeID,Title,Description,MobileSourceFile,DesktopSourceFile,RowPosition,ColumnPosition,RowSpan,ColumnSpan,CreatorID,CreatedDate,ModifierID,ModifiedDate,Permission,PermissionGroup,State,DeletedDate ) values ( @EntityID,@ItemID,@ParentID,@TypeID,@Title,@Description,@MobileSourceFile,@DesktopSourceFile,@RowPosition,@ColumnPosition,@RowSpan,@ColumnSpan,@CreatorID,@CreatedDate,@ModifierID,@ModifiedDate,@Permission,@PermissionGroup,@State,@DeletedDate )", myConnection)
			...
			Return entityID
		End Function
		Public Overridable Sub UpdateEntity(ByVal entityID As String, ByVal parentID As String, ByVal typeID As Integer, ByVal title As String, ByVal description As String, ByVal mobileSourceFile As String, ByVal desktopSourceFile As String, ByVal rowPosition As Integer, ByVal columnPosition As Integer, ByVal rowSpan As Integer, ByVal columnSpan As Integer, ByVal creatorID As String, ByVal createdDate As Date, ByVal modifierID As String, ByVal modifiedDate As Date, ByVal permission As String, ByVal permissionGroup As String, ByVal state As Integer, ByVal deletedDate As Date)
			Dim myConnection As New SqlConnection(ConfigurationSettings.AppSettings("AuditSystemConnectionString"))
			Dim myCommand As New SqlCommand("update ControlDefinition set ParentID=@ParentID , TypeID=@TypeID , Title=@Title , Description=@Description , MobileSourceFile=@MobileSourceFile , DesktopSourceFile=@DesktopSourceFile , RowPosition=@RowPosition , ColumnPosition=@ColumnPosition , RowSpan=@RowSpan , ColumnSpan=@ColumnSpan , CreatorID=@CreatorID , CreatedDate=@CreatedDate , ModifierID=@ModifierID , ModifiedDate=@ModifiedDate , Permission=@Permission , PermissionGroup=@PermissionGroup , State=@State , DeletedDate=@DeletedDate where EntityID=@EntityID", myConnection)
			myCommand.Parameters.Add("@ParentID", SqlDbType.NVarChar, 16).Value = parentID
			myCommand.Parameters.Add("@TypeID", SqlDbType.Int, 4).Value = typeID
			myCommand.Parameters.Add("@Title", SqlDbType.NVarChar, 64).Value = title
			myCommand.Parameters.Add("@Description", SqlDbType.NVarChar, 256).Value = description
		...
		Public Overridable Function GetMaxEntityID(ByVal entityID As String) As String
			...
			selectSQL = "select max(EntityID) from ControlDefinition where substring(EntityID,1,8)='" & entityID.substring(0, 8) & "'"
		...
		Public Overridable Function GetTotalRow() As Integer
			Dim valResult As Integer
			Dim myConnection As New SqlConnection(ConfigurationSettings.AppSettings("AuditSystemConnectionString"))
			Dim myCommand As New SqlCommand("select count(*) from ControlDefinition", myConnection)
			myConnection.Open()
			Dim myReader As SqlDataReader
			myReader = myCommand.ExecuteReader(CommandBehavior.CloseConnection)
			While myReader.Read()
