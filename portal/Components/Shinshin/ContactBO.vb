...
Public Class ContactBO
    ...
    Public Function Query(Optional ByVal NO As String = "") As DataTable
        ...
        Dim dt As DataTable
        strSQL = "Select *,isnull(workdate,'') workdate1 ,CASE workstatus WHEN '0' THEN '未指派' WHEN '1' THEN '處理中' WHEN '2' THEN '已處理' WHEN '3' THEN '已辦結' END as status ,CASE cnttype  WHEN '1' THEN '一般服務' WHEN '2' THEN '漏氣報修' WHEN '3' THEN '簡訊服務' END as cnttype_name  from contact as A left join portal.dbo.vUserInfo as B on A.assignoperator = B.UID "
        If NO <> "" Then
            strSQL &= "where cntno='" & NO & "' "
    ...
    '一般查詢(沒比對登入者帳號) 只查詢漏氣搶修
    Public Function Query_2(Optional ByVal NO As String = "") As DataTable
        Dim strSQL As String = ""
        Dim dt As DataTable
        strSQL = "Select *,isnull(workdate,'') workdate1 ,CASE workstatus WHEN '0' THEN '未指派' WHEN '1' THEN '處理中' WHEN '2' THEN '已處理' WHEN '3' THEN '已辦結' END as status ,CASE cnttype  WHEN '1' THEN '一般服務' WHEN '2' THEN '漏氣報修' WHEN '3' THEN '簡訊服務' END as cnttype_name  from contact as A left join portal.dbo.vUserInfo as B on A.assignoperator = B.UID "
        If NO <> "" Then
    ...
    Public Function UserQuery(ByVal userId As String, ByVal startDate As String, ByVal endDate As String, ByVal status As String, ByVal cnttype As String, ByVal num As Integer, ByVal likeSel As String, ByVal likecontent As String, ByVal identity As String) As DataTable
        Dim strSQL As String = ""
        ...
        '沒有過濾登入者
        strSQL = "Select *,isnull(workdate,'') workdate1 ,CASE workstatus WHEN '0' THEN '未指派' WHEN '1' THEN '處理中' WHEN '2' THEN '已處理' WHEN '3' THEN '已辦結' END as status ,CASE cnttype  WHEN '1' THEN '一般服務' WHEN '2' THEN '漏氣報修' WHEN '3' THEN '簡訊服務' END as cnttype_name  fro

m contact as A left join portal.dbo.vUserInfo as B on A.assignoperator = B.UID  "
        If startDate = "" Then
            startDate = "1911-01-01 00:00:00"
        End If
        If endDate = "" Then
            endDate = "2911-12-31 00:00:00"
        End If
        '判斷查詢日期條件
        If num = 1 Then
        ...
        End If
        '判斷意見類別
        If cnttype <> "9" Then  '9==>查全部狀態
            strSQL &= "and cnttype ='" & cnttype & "'"
        End If
        '判斷登入者身分
    ...
    Public Function Update(ByVal Ne As Encontact) As Integer
        ...
        End If
        '初次處理人員
        If Ne.myoperator <> "" Then
            strSQLBody &= ",operator='" & Ne.myoperator & "'"
        End If
        '初次處理時間
        If Ne.workdate <> "" Then
