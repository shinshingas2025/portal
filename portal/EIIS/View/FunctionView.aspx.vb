...
#Region " Web Form 設計工具產生的程式碼 "
    ...
    End Sub
    Protected WithEvents txtFunctionID As System.Web.UI.WebControls.TextBox
    Protected WithEvents txtDescription As System.Web.UI.WebControls.TextBox
    Protected WithEvents txtExeFileName As System.Web.UI.WebControls.TextBox
    Protected WithEvents txtLogicalFilePath As System.Web.UI.WebControls.TextBox
    Protected WithEvents txtExeCMDLine As System.Web.UI.WebControls.TextBox
    Protected WithEvents btnOK As System.Web.UI.WebControls.Button
    Protected WithEvents txtSeqno As System.Web.UI.WebControls.TextBox
    Protected WithEvents txPanelName As System.Web.UI.WebControls.DropDownList
    Protected WithEvents txtModuleType As System.Web.UI.WebControls.DropDownList
...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Not IsPostBack Then
            fillModuleCombo()
            txtFunctionID.Text = txtModuleType.SelectedItem.Text
        End If


    End Sub

    Private Sub fillModuleCombo()
        Dim se As New SecurityDB
        Dim dt As DataTable
        Dim i As Integer
        dt = se.GetALLModuleExceptSM().Tables(0)


        For i = 0 To dt.Rows.Count - 1

            Dim it As New ListItem
            it.Text = CType(dt.Rows(i).Item("FriendlyName"), String)
            it.Value = CType(dt.Rows(i).Item("ModuleDefId"), String)
            txtModuleType.Items.Add(it)
        Next
    ...
    Private Sub InsertFunction()
        Dim node As New EIIS.Functions
        Dim dm As New Domains

        node.FunctionID = txtFunctionID.Text.Trim
        node.ExeFileName = txtExeFileName.Text.Trim
        node.ExeCMDLine = txtExeCMDLine.Text.Trim
        node.LogicalFilePath = txtLogicalFilePath.Text.Trim
        node.Description = txtDescription.Text.Trim
        node.PID = CType(Session("DomainNodePID"), Integer)

        node.objName = txtFunctionID.Text.Trim
        node.DataType = "Function"
        node.srcName = "Function"
        node.SEQNO = txtSeqno.Text.Trim
        node.state = ""

        node.PaneName = txPanelName.SelectedValue
        node.ModuleDefid = CType(txtModuleType.SelectedValue, Integer)

        dm.InsertFunction(node)
    ...
    Private Sub txtModuleType_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtModuleType.SelectedIndexChanged
        txtFunctionID.Text = txtModuleType.SelectedItem.Text
    End Sub
End Class
