...
<%
  Dim objConn, rstCat, rstBoard, dBoardLV, dLV
  ...
  rstCat.CursorLocation = adUseClient
  Set rstBoard = Server.CreateObject("ADODB.RecordSet")
  rstBoard.CursorLocation = adUseClient

  rstCat.Open "SELECT [CID], [Title] FROM [Categories] ORDER BY [Title]", objConn, adOpenKeyset, adLockReadOnly, adCmdText
%>
...
%>
        <tr class="category">
          <td colspan="5"><font class="category"><b><% = rstCat("Title").Value %></b></font></td>
        </tr>
<%
    rstBoard.Open "SELECT [BID], [Title], [Description], [TotalPosts], [TotalTopics], [LastUpdateDate] FROM [Boards] WHERE [Category] = " & rstCat("CID") & " ORDER BY [Position]", objConn, adOpenKeyset, adLockReadOnly, adCmdText
