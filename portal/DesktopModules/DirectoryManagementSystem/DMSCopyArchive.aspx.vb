			...
			If targetFolderID.Trim.Length > 0 Then
				...
				If myFolderDataSet.Tables(0).Rows.Count = 1 Then
					...
					If TextBoxPassword.Text.Trim = myFolderPassword.Trim Then
						If fileID.Trim.Length > 0 And folderID.Trim.Length > 0 Then
							myFileDataSet = myFileDAO.GetEntitysByEntityID(fileID)
							'check if file record is exist
							If myFileDataSet.Tables(0).Rows.Count = 1 Then
								myName = CType(myFileDataSet.Tables(0).Rows(0).Item("Name"), String)
								myFileCount = myFileDAO.GetTotalRowByFolderIDAndName(targetFolderID, myName)
								'check if copy duplicated
								If myFileCount = 0 Then
									myFileCount = myFileDAO.GetTotalRowByFolderIDAndName(folderID, myName)
									If myFileCount > 0 Then
										'copy file record
										myFileDataSet = myFileDAO.GetEntitysByFolderIDAndName(folderID, myName)
										For i = 0 To myFileCount - 1
											...
											myFileDAO.InsertEntity(0, myName, myPhysicalFileName, myFileSize, targetFolderID, myDescription, myMetaData, myPermission, myOwner, myMajorRevision, myMinorRevision, myURL, myPassword, myDocumentTypeID, "", "", 0, 0, Now, Now)
										Next
									End If
								Else
									'name duplicated
									LabelResult.Text = "檔名重複!"
								End If
							Else
