...
#End Region
		...
		Private Sub ButtonOK_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonOK.Click
						...
						If folderID.Trim.Length > 0 Then
							...
							If myFolderDataSet.Tables(0).Rows.Count = 1 Then
								...
								If myFolderCount = 0 Then
									CopyDirectory(folderID, targetFolderID)
								Else
									'name duplicated
			...
			End If
		End Sub
		Private Sub CopyDirectory(ByVal originDirectoryID As String, ByVal targetDirectoryID As String)
			Dim myFolderDAO As New DMS_FolderDAOExtand
			Dim myFolderDataSet As DataSet
			Dim myFolderCount As Integer
			Dim myFileDAO As New DMS_FileDAOExtand
			...
			If originDirectoryID.Trim.Length > 0 And targetDirectoryID.Trim.Length > 0 Then
				myFolderDataSet = myFolderDAO.GetEntitysByEntityID(originDirectoryID)
				If myFolderDataSet.Tables(0).Rows.Count = 1 Then
					myName = CType(myFolderDataSet.Tables(0).Rows(0).Item("Name"), String)
					...
					myParentID = myFolderDAO.InsertEntity(0, myName, targetDirectoryID, myDescription, myPermission, myPassword, 0, 0, Now, Now, myOwner)
					'copy subdirectory
					myFolderCount = myFolderDAO.GetTotalRowByParentID(originDirectoryID)
					If myFolderCount > 0 Then
						myFolderDataSet = myFolderDAO.GetEntitysByParentID(originDirectoryID)
						For i = 0 To myFolderCount - 1
							myFolderID = CType(myFolderDataSet.Tables(0).Rows(i).Item("EntityID"), String)
							CopyDirectory(myFolderID, myParentID)
						Next
					End If
					'copy file
					myFileCount = myFileDAO.GetTotalRowByFolderID(originDirectoryID)
					If myFileCount > 0 Then
						myFileDataSet = myFileDAO.GetEntitysByFolderID(originDirectoryID)
						For i = 0 To myFileCount - 1
							myDocumentTypeID = CType(myFileDataSet.Tables(0).Rows(i).Item("DocumentTypeID"), String)
							...
							myURL = CType(myFileDataSet.Tables(0).Rows(i).Item("URL"), String)
							myName = CType(myFileDataSet.Tables(0).Rows(i).Item("Name"), String)
							myMetaData = CType(myFileDataSet.Tables(0).Rows(i).Item("MetaData"), String)
							...
							myOwner = CType(myFileDataSet.Tables(0).Rows(i).Item("GroupID"), Integer)
							myPermission = CType(myFileDataSet.Tables(0).Rows(i).Item("Permission"), String)
							myPassword = CType(myFileDataSet.Tables(0).Rows(i).Item("Password"), String)
							myDescription = CType(myFileDataSet.Tables(0).Rows(i).Item("Description"), String)

							myPhysicalFileName = ""
							identityKey = Now.Year & Microsoft.VisualBasic.Right("00" & Now.Month, 2) & Microsoft.VisualBasic.Right("00" & Now.Day, 2) & Microsoft.VisualBasic.Right("00" & Now.Hour, 2) & Microsoft.VisualBasic.Right("00" & Now.Minute, 2) & Microsoft.VisualBasic.Right("00" & Now.Second, 2) & Microsoft.VisualBasic.Right("000" & Now.Millisecond, 3)
							'copy upload file
							If myFileName.Trim.Length > 0 Then
								myPhysicalFileName = Server.MapPath("/PortalFiles/UpLoadFiles/DMS") & "/" & identityKey & Path.GetFileName(myFileName).Substring(17)
								If File.Exists(myPhysicalFileName) Then
									File.Delete(myPhysicalFileName)
								End If
								If File.Exists(myFileName) Then
									File.Copy(myFileName, myPhysicalFileName)
								End If
							End If

							myFileDAO.InsertEntity(0, myName, myPhysicalFileName, myFileSize, myParentID, myDescription, myMetaData, myPermission, myOwner, myMajorRevision, myMinorRevision, myURL, myPassword, myDocumentTypeID, "", "", 0, 0, Now, Now)
						Next
					End If
				Else
					'exception:origin directory record is empty or duplicated
				End If
			Else
				'exception:origin directory id is empty
			End If
		End Sub
	End Class
End Namespace