...
#End Region
		...
		Private Sub PageSave()
			...
			Dim myMajorRevision As Integer = 0
			Dim myMinorRevision As Integer = 0
			Dim myOldMajorRevision As Integer = 0
			Dim myOldMinorRevision As Integer = 0
			Dim myName As String = ""
			...
			If fileID.Trim.Length > 0 Then
				...
				myFileDataSet = myFileDAO.GetEntitysByEntityID(fileID)
				If myFileDataSet.Tables(0).Rows.Count = 1 Then
					myDocumentType = CType(myFileDataSet.Tables(0).Rows(0).Item("DocumentTypeID"), String)
					myName = CType(myFileDataSet.Tables(0).Rows(0).Item("Name"), String)
					myOwner = CType(myFileDataSet.Tables(0).Rows(0).Item("GroupID"), Integer)
					...
					myOldMajorRevision = CType(myFileDataSet.Tables(0).Rows(0).Item("MajorRevision"), Integer)
					myOldMinorRevision = CType(myFileDataSet.Tables(0).Rows(0).Item("MinorRevision"), Integer)
					'check version
					If myOldMajorRevision + myOldMinorRevision * 1000 <> myMajorRevision + myMinorRevision * 1000 Then
						'check password
						If myOldPassword.Trim = TextBoxPassword.Text.Trim Then
							'save upload file
							If Not (myFile Is Nothing) Then
								...
								End If
							End If
							myFileDAO.InsertEntity(0, myName, myPhysicalFileName, myFileSize, myOldFolderID, myDescription, myMetaData, myPermission, myOwner, myMajorRevision, myMinorRevision, myURL, myOldPassword, myDocumentType, "", "", 0, 0, Now, Now)
						Else
