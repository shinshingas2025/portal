...
#End Region
		...
		Private Sub PageLoad()
			...
			If myQuestionDataSet.Tables(0).Rows.Count = 1 Then
				TextBoxQuestionAlias.Text = CType(myQuestionDataSet.Tables(0).Rows(0).Item("QuestionAlias"), String)
				txtQuestion.Text = CType(myQuestionDataSet.Tables(0).Rows(0).Item("QuestionText"), String)
				choice = CType(myQuestionDataSet.Tables(0).Rows(0).Item("SelectModeID"), Integer)
				If choice = SelectMode.MultipleSelect Then
					RadioButtonMultiple.Checked = True
				End If
				myEnableDate = CType(myQuestionDataSet.Tables(0).Rows(0).Item("EnableDate"), Date)
				myDisableDate = CType(myQuestionDataSet.Tables(0).Rows(0).Item("DisableDate"), Date)
				TextBox1.Text = myEnableDate.Year & "/" & Microsoft.VisualBasic.Right("00" & myEnableDate.Month, 2) & "/" & Microsoft.VisualBasic.Right("00" & myEnableDate.Day, 2)
				TextBox2.Text = myDisableDate.Year & "/" & Microsoft.VisualBasic.Right("00" & myDisableDate.Month, 2) & "/" & Microsoft.VisualBasic.Right("00" & myDisableDate.Day, 2)

			Else
				'exception
			End If

			If myAnswerDataset.Tables.Count = 1 Then
				For i = 0 To myAnswerDataset.Tables(0).Rows.Count - 1
					listAnswers.Items.Add(CType(myAnswerDataset.Tables(0).Rows(i).Item("AnswerAlias"), String).Trim)
				Next
			End If
		...
		Private Sub SaveQuestion()
			...
			If questionID.Trim.Length > 0 Then
				'check select mode
				...
				'save question
				'audit
				myAuditID = AuditDAO.InsertEntity(sid, moduleId, 0, LevelType.info, ActionType.update, Me.ToString, myQuestion.ToString, "UpdateEntity", questionID, "", Context.User.Identity.Name, Now)
				'log before action
				myQuestionDataSet = myQuestion.GetEntitys(questionID)
				If myQuestionDataSet.Tables(0).Rows.Count = 1 Then
				...
				End If
				'actual action
				myQuestion.UpdateEntity(questionID, sid, moduleId, "", mySelectMode, ShowMode.Horizontal, 0, 0, 1, TextBoxQuestionAlias.Text.Trim, txtQuestion.Text.Trim, myEnableDate, myDisableDate, user, Now)
				'log after action
				myQuestionDataSet = myQuestion.GetEntitys(questionID)
				If myQuestionDataSet.Tables(0).Rows.Count = 1 Then
					AuditDetail(myAuditID, SequenceType.before, myQuestionDataSet)
				...
				For i = 0 To listAnswers.Items.Count - 1
					...
					'none
					'actual action
					myAnswerID = myAnswer.InsertEntity(questionID, 0, AnswerType.RadioButton, i + 1, 0, myText, myText, "", user, Now)
					'log after action
		...
		Private Sub imageRight_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imageRight.Click
			If txtAnswer.Text.Trim <> "" Then
				listAnswers.Items.Add(txtAnswer.Text.Trim)
			End If
		End Sub
