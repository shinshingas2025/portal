...
#End Region
		...
		Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
			...
			If Not IsPostBack Then
				...
				If queryColumn = "ResolutionContent" Then
					rowCount = myRecordDAO.GetTotalRowByQueryString(queryString)
				Else
					If queryColumn = "ProcessState" Then
						rowCount = rowCount + myFormDAO.GetTotalRowByQueryStringInAssociation(queryString)
						rowCount = rowCount + myFormDAO.GetTotalRowByQueryStringInCouncil(queryString)
						rowCount = rowCount + myFormDAO.GetTotalRowByQueryStringInBureau(queryString)
						rowCount = rowCount + myFormDAO.GetTotalRowByQueryStringInSection(queryString)
					Else
		...
		Private Sub PageLoad()
			...
			If queryColumn = "ResolutionContent" Then
				myRecordDataSet = myRecordDAO.GetEntitysByQueryString(queryString, pageSize * currentEntitysListPage)
			Else
				If queryColumn = "ProcessState" Then
					myFormDataSet = myFormDAO.GetEntitysByQueryStringInAssociation(queryString)
					myFormDataSet.Merge(myFormDAO.GetEntitysByQueryStringInCouncil(queryString))
					myFormDataSet.Merge(myFormDAO.GetEntitysByQueryStringInBureau(queryString))
					myFormDataSet.Merge(myFormDAO.GetEntitysByQueryStringInSection(queryString))
				...
				If myRecordDataSet.Tables(0).Rows.Count > 0 Then
					...
					For i = lowBound To highBound
						...
						Else
							...
							myHyperLink.NavigateUrl = "~/DesktopModules/AuditSystem/DatabaseQuery.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&QueryColumn=" & queryColumn & "&QueryString=" & queryString & "&CurrentEntitysListPage=" & CType(i, String)
					...
					''prepare entitys data
					'If myEntitysDataSet.Tables(0).Rows.Count > 0 Then
					'	For i = (currentEntitysListPage - 1) * pageSize To myEntitysDataSet.Tables(0).Rows.Count - 1
					'		'categorization name
					'		myCategorizationID = CType(myEntitysDataSet.Tables(0).Rows(i).Item("CategorizationID"), String).Trim
					'		If myCategorizationID.Length > 0 Then
					'			myCategorizationName = myNormalCodeDAO.GetNameByEntityID(myCategorizationID)
					'			myEntitysDataSet.Tables(0).Rows(i).Item("CategorizationName") = myCategorizationName
					'		End If
					'		'entitys date string
					'		myEntitysDate = CType(myEntitysDataSet.Tables(0).Rows(i).Item("EntitysDate"), Date)
					'		If myEntitysDate = myEmptyDate Then
					'			myEntitysDataSet.Tables(0).Rows(i).Item("EntitysDateString") = ""
					'		Else
					'			myEntitysDataSet.Tables(0).Rows(i).Item("EntitysDateString") = myEntitysDate.Year & "/" & myEntitysDate.Month & "/" & myEntitysDate.Day
					'		End If
					'	Next
					'End If
					...
					If myRecordDataSet.Tables(0).Rows.Count > 0 Then
						For i = (currentEntitysListPage - 1) * pageSize To myRecordDataSet.Tables(0).Rows.Count - 1
							...
							myHyperLink.NavigateUrl = "~/DesktopModules/AuditSystem/MeetingRecordAdmin.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&typeID=" & myMeetingTypeID & "&recordID=" & myEntityID
					...
					If myFormDataSet.Tables(0).Rows.Count > 0 Then
						...
						If myFormDataSet.Tables(0).Rows.Count > 0 Then
							For i = (currentEntitysListPage - 1) * pageSize To myFormDataSet.Tables(0).Rows.Count - 1
								...
								myHyperLink.ImageUrl = "~/images/link.gif"
								myHyperLink.NavigateUrl = "~/DesktopModules/AuditSystem/AffairProcessCheckForm.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&groupID=" & myGroupID & "&formID=" & myEntityID
								myDetailHtmlTableCell.Controls.Add(myHyperLink)
								myDetailHtmlTableRow.Cells.Add(myDetailHtmlTableCell)
								myDetailHtmlTable.Controls.Add(myDetailHtmlTableRow)
		...
		Private Sub ButtonOK_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonOK.Click
			...
			Response.Redirect("~/DesktopModules/AuditSystem/DatabaseQuery.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&QueryColumn=" & myQueryColumn & "&QueryString=" & myQueryString)
		End Sub
	End Class
End Namespace
