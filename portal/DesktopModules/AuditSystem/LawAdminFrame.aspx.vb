...
#End Region
		...
		Private Const LawContentAuthorityTarget As String = "LawContent"
		Private Const LawAppendAuthorityTarget As String = "LawAppend"
		...
		Private Sub PageLoad()
			...
			Else
				If parentID.Trim.Length > 0 Then
					myLawDataSet = myLawDAO.GetEntitysByParentID(parentID)
					myLawDataSet = UtilityObject.QueryPermissionFilter(myLawDataSet, LawAuthorityTarget, Context.User.Identity.Name, 1)

					If myLawDataSet.Tables(0).Rows.Count = 1 Then
						myLawID = CType(myLawDataSet.Tables(0).Rows(0).Item("EntityID"), String)
						Response.Redirect("~/DesktopModules/AuditSystem/LawAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&parentID=" & parentID & "&lawID=" & myLawID)
					End If
				Else
					'no parent id
					...
					If myLawDataSet.Tables(0).Rows.Count = 1 Then
						...
						Response.Redirect("~/DesktopModules/AuditSystem/LawAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&parentID=" & myParentID & "&lawID=" & myLawID)
					End If
				End If
			End If
		...
		Private Sub FillContentData(ByVal myLawID As String)
			...
			If myLawID.Trim.Length > 0 Then
				...
				If myContentCount > 0 Then
					...
					myContentDataSet = UtilityObject.QueryPermissionFilter(myContentDataSet, LawContentAuthorityTarget, Context.User.Identity.Name)
					myContentCount = myContentDataSet.Tables(0).Rows.Count
					...
					For i = 0 To myContentCount - 1
						...
						myHyperLink.NavigateUrl = "~/DesktopModules/AuditSystem/LawAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&parentID=" & parentID & "&lawID=" & myLawID & "&contentID=" & myContentID & "&action=update"
						...
						'insert link
						myHyperLink = New HyperLink
						myHyperLink.NavigateUrl = "~/DesktopModules/AuditSystem/LawAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&parentID=" & parentID & "&lawID=" & myLawID & "&contentID=" & myContentID & "&action=delete"
		...
		Private Sub FillAppendData(ByVal myLawID As String)
			...
			If myLawID.Trim.Length > 0 Then
				...
				If myAppendCount > 0 Then
					...
					myAppendDataSet = UtilityObject.QueryPermissionFilter(myAppendDataSet, LawAppendAuthorityTarget, Context.User.Identity.Name)
					myAppendCount = myAppendDataSet.Tables(0).Rows.Count
					...
					For i = 0 To myAppendCount - 1
						...
						'insert link
						myHyperLink = New HyperLink
						myHyperLink.NavigateUrl = "~/DesktopModules/AuditSystem/LawAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&parentID=" & parentID & "&lawID=" & myLawID & "&appendID=" & myAppendID & "&action=update"
						myHyperLink.ImageUrl = "~/images/edit.gif"
						myHyperLink.Text = "修改"
						myTableCell.Controls.Add(myHyperLink)
						'br
						...
						'insert link
						myHyperLink = New HyperLink
						myHyperLink.NavigateUrl = "~/DesktopModules/AuditSystem/LawAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&parentID=" & parentID & "&lawID=" & myLawID & "&appendID=" & myAppendID & "&action=delete"
						myHyperLink.ImageUrl = "~/images/delete.gif"
						myHyperLink.Text = "刪除"
						myTableCell.Controls.Add(myHyperLink)
		...
		Private Function SaveLawData(ByVal myLawID As String) As String
			...
			If myLawID.Trim.Length > 0 Then
				...
				If myLawDataSet.Tables(0).Rows.Count = 1 Then
					'get authority
					If myAuthorityBO.CheckPurview(LawAuthorityTarget, myLawID, Context.User.Identity.Name, "U") Then
						'update law record
						myLawDAO.UpdateEntity(myLawID, myName, myDiscussionID, myConstitutionDate, myParentID, myVariationTypeID, myConstitutionInstitutionID, myUndertakerInstitutionID, myDocumentNumber, myTypeID, Context.User.Identity.Name, Now)
					End If
				Else
					'exception:law record is empty or duplicated
				End If
			Else
				'insert law data
				myLawID = myLawDAO.InsertEntity(0, myName, myDiscussionID, myConstitutionDate, myParentID, Context.User.Identity.Name, Now, Context.User.Identity.Name, Now, DefaultPermission, DefaultPermissionGroup, 1, New Date(1900, 1, 1), myVariationTypeID, myConstitutionInstitutionID, myUndertakerInstitutionID, myDocumentNumber, myTypeID)
			End If
			Return myLawID
		End Function
			...
			If lawID.Trim.Length > 0 Then
				...
				If myLawDataSet.Tables(0).Rows.Count = 1 Then
					...
					If myLawDataSet.Tables(0).Rows.Count > 0 Then
						...
						Next
						If bFound = True Then
							Response.Redirect("~/DesktopModules/AuditSystem/LawAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&parentID=" & parentID & "&lawID=" & myPreviousID)
						End If
					End If
				Else
				...
				If myLawDataSet.Tables(0).Rows.Count = 1 Then
					...
					If myLawDataSet.Tables(0).Rows.Count > 0 Then
						...
						If bFound = True Then
							...
							End If
							Response.Redirect("~/DesktopModules/AuditSystem/LawAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&parentID=" & parentID & "&lawID=" & myNextID)
						End If
					End If
				Else
					'exception:record is empty or duplicated
		...
		Private Sub ButtonEntityUpdate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonEntityUpdate.Click
			If lawID.Trim.Length > 0 Then
				SaveLawData(lawID)
				Response.Redirect("~/DesktopModules/AuditSystem/LawAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&parentID=" & parentID & "&lawID=" & lawID)
			Else
			...
			If myLawID.Trim.Length > 0 Then
				...
				If myContentCount > 0 Then
					...
					For i = 0 To myContentCount - 1
						myContentID = CType(myContentDataSet.Tables(0).Rows(i).Item("EntityID"), String)
						'get authority
						If myAuthorityBO.CheckPurview(LawContentAuthorityTarget, myContentID, Context.User.Identity.Name, "D") Then
			...
			If myLawID.Trim.Length > 0 Then
				...
				If myAppendCount > 0 Then
					...
					For i = 0 To myAppendCount - 1
						...
						myAppendFile = CType(myAppendDataSet.Tables(0).Rows(i).Item("FileName"), String)
						'get authority
						If myAuthorityBO.CheckPurview(LawAppendAuthorityTarget, myAppendID, Context.User.Identity.Name, "D") Then
							'delete append file
		...
		Private Sub DeleteLawData(ByVal myLawID As String)
			...
			If myLawID.Trim.Length > 0 Then
				...
				If myLawDataSet.Tables(0).Rows.Count = 1 Then
					...
					'get authority
					If myAuthorityBO.CheckPurview(LawAuthorityTarget, myLawID, Context.User.Identity.Name, "D") Then
						'delete law
		...
		Private Sub ButtonEntityDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonEntityDelete.Click
			If lawID.Trim.Length > 0 Then
				...
				lawID = ""
				Response.Redirect("~/DesktopModules/AuditSystem/LawAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&parentID=" & parentID)
			Else
				'exception:law id is empty
			End If
		End Sub
		Private Sub ButtonContentAction_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonContentAction.Click
			...
			If lawID.Trim.Length > 0 Then
				...
				If contentID.Trim.Length > 0 Then
					If action.Trim.Length > 0 Then
						If action.Trim = "update" Then
							...
							If myContentDataSet.Tables(0).Rows.Count = 1 Then
								'get authority
								If myAuthorityBO.CheckPurview(LawContentAuthorityTarget, contentID, Context.User.Identity.Name, "U") Then
									'actual action
									myContentDAO.UpdateEntity(contentID, lawID, myContentNumber, myContent, myContentOrder, Context.User.Identity.Name, Now)
									Response.Redirect("~/DesktopModules/AuditSystem/LawAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&parentID=" & parentID & "&lawID=" & lawID)
								Else
									PageLoad()
								End If
						...
						Else
							If action.Trim = "delete" Then
								...
								If myContentDataSet.Tables(0).Rows.Count = 1 Then
									'get authority
									If myAuthorityBO.CheckPurview(LawContentAuthorityTarget, contentID, Context.User.Identity.Name, "D") Then
										'actual action
				...
				Else
					'insert new content
					myContentDAO.InsertEntity(lawID, 0, myContentNumber, myContent, myContentOrder, Context.User.Identity.Name, Now, Context.User.Identity.Name, Now, DefaultPermission, DefaultPermissionGroup, 1, New Date(1900, 1, 1))
					Response.Redirect("~/DesktopModules/AuditSystem/LawAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&parentID=" & parentID & "&lawID=" & lawID)
				End If
			Else
				'exception:law id is empty
		...
		Private Sub ButtonAppendAction_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonAppendAction.Click
			...
			If lawID.Trim.Length > 0 Then
				...
				If appendID.Trim.Length > 0 Then
					If action.Trim.Length > 0 Then
						If action.Trim = "update" Then
							...
							If myAppendDataSet.Tables(0).Rows.Count = 1 Then
								'get authority
								If myAuthorityBO.CheckPurview(LawAppendAuthorityTarget, appendID, Context.User.Identity.Name, "U") Then
									'update append file
									...
									End If
									'actual action
									myAppendDAO.UpdateEntity(appendID, lawID, myAppendName, myAppendDescription, myPhysicalFileName, myFileSize, myAppendOrder, Context.User.Identity.Name, Now)
									Response.Redirect("~/DesktopModules/AuditSystem/LawAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&parentID=" & parentID & "&lawID=" & lawID)
								Else
									PageLoad()
								End If
						...
						Else
							If action.Trim = "delete" Then
								...
								If myAppendDataSet.Tables(0).Rows.Count = 1 Then
									'get authority
									If myAuthorityBO.CheckPurview(LawAppendAuthorityTarget, appendID, Context.User.Identity.Name, "D") Then
										'delete append file
										myOldFileName = CType(myAppendDataSet.Tables(0).Rows(0).Item("FileName"), String)
										If File.Exists(myOldFileName) Then
											File.Delete(myOldFileName)
				...
				Else
					...
					End If
					'actual action
					myAppendDAO.InsertEntity(lawID, 0, myAppendName, myAppendDescription, myPhysicalFileName, myFileSize, myAppendOrder, Context.User.Identity.Name, Now, Context.User.Identity.Name, Now, DefaultPermission, DefaultPermissionGroup, 1, New Date(1900, 1, 1))
					Response.Redirect("~/DesktopModules/AuditSystem/LawAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&parentID=" & parentID & "&lawID=" & lawID)
				End If
			Else
				'exception:news id is empty
			...
			If lawID.Trim.Length > 0 Then
				...
				If myLawDataSet.Tables(0).Rows.Count = 1 Then
					...
					Else
						...
						myParentID = "0"
					End If
					Response.Redirect("~/DesktopModules/AuditSystem/LawAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&parentID=" & myParentID & "&lawID=" & myPreviousID)
				Else
					'exception:record is empty or duplicated
				End If
			Else
		...
		Private Sub ButtonChild_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonChild.Click
			...
			If lawID.Trim.Length > 0 Then
				...
				If myLawDataSet.Tables(0).Rows.Count = 1 Then
					'read one of child
					myLawDataSet = myLawDAO.GetEntitysByParentID(lawID)
					myLawDataSet = UtilityObject.QueryPermissionFilter(myLawDataSet, LawAuthorityTarget, Context.User.Identity.Name, 1)
					...
					If myLawDataSet.Tables(0).Rows.Count = 1 Then
						...
						myParentID = CType(myLawDataSet.Tables(0).Rows(0).Item("ParentID"), String)
					End If
					Response.Redirect("~/DesktopModules/AuditSystem/LawAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&parentID=" & myParentID & "&lawID=" & myNextID)
				Else
					'exception:record is empty or duplicated
				End If
			Else
