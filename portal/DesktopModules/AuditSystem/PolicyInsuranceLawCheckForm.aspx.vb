...
Namespace ASPNET.StarterKit.Portal.AuditSystem.Module
	...
	Public Class PolicyInsuranceLawCheckForm
...
#End Region
		...
		Private Sub PageLoad()
			...
			Else
				myFormCount = myFormDAO.GetTotalRow()
				If myFormCount > 0 Then
					myFormDataSet = myFormDAO.GetEntitys(1)
					If myFormDataSet.Tables(0).Rows.Count = 1 Then
						myFormID = CType(myFormDataSet.Tables(0).Rows(0).Item("EntityID"), String)
						Response.Redirect("~/DesktopModules/AuditSystem/PolicyInsuranceLawCheckForm.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&formID=" & myFormID & "&commentID=" & commentID & "&insuranceID=" & insuranceID)
					End If
				End If
			End If
		...
		Private Sub FillInsuranceData(ByVal myFormID As String)
			...
			If myFormID.Trim.Length > 0 Then
				...
				Else
					myInsuranceMapDataSet = myInsuranceMapDAO.GetEntitysByPolicyLawID(myFormID, 1)
					If myInsuranceMapDataSet.Tables(0).Rows.Count = 1 Then
						...
						Response.Redirect("~/DesktopModules/AuditSystem/PolicyInsuranceLawCheckForm.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&formID=" & myFormID & "&commentID=" & commentID & "&insuranceID=" & myInsuranceMapID)
					End If
				End If
				If myInsuranceMapDataSet.Tables(0).Rows.Count = 1 Then
			...
			If myFormID.Trim.Length > 0 Then
				...
				If myCommentCount > 0 Then
					...
					For i = 0 To myCommentCount - 1
						...
						myHyperLink.NavigateUrl = "~/DesktopModules/AuditSystem/PolicyInsuranceLawCheckForm.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&formID=" & myFormID & "&commentID=" & myCommentID & "&insuranceID=" & insuranceID & "&commentAction=update"
						...
						'insert link
						myHyperLink = New HyperLink
						myHyperLink.NavigateUrl = "~/DesktopModules/AuditSystem/PolicyInsuranceLawCheckForm.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&formID=" & myFormID & "&commentID=" & myCommentID & "&insuranceID=" & insuranceID & "&commentAction=delete"
		...
		Private Sub InitialWebControl()
			...
			myMeetingRecordCount = myMeetingRecordDAO.GetTotalRowByTypeID(PolicyMeetingRecordTypeID)
			...
			myNormalCodeCount = myNormalCodeDAO.GetTotalRowByGroupID(ManagementInstitutionCodeGroupID)
			...
			myNormalCodeCount = myNormalCodeDAO.GetTotalRowByGroupID(PolicyLawUndertakerCodeGroupID)
			If myNormalCodeCount > 0 Then
			...
			myNormalCodeCount = myNormalCodeDAO.GetTotalRowByGroupID(SignCodeGroupID)
			If myNormalCodeCount > 0 Then
			...
			myNormalCodeCount = myNormalCodeDAO.GetTotalRowByGroupID(DraftCodeGroupID)
			If myNormalCodeCount > 0 Then
			...
			myNormalCodeCount = myNormalCodeDAO.GetTotalRowByGroupID(MemberCodeGroupID)
			If myNormalCodeCount > 0 Then
			...
			myInsuranceCodeCount = myInsuranceCodeDAO.GetTotalRow()
			...
			myNormalCodeCount = myNormalCodeDAO.GetTotalRowByGroupID(PolicyInsuranceUndertakerCodeGroupID)
			If myNormalCodeCount > 0 Then
		...
		Private Function SaveFormData(ByVal myFormID As String) As String
			...
			If myFormID.Trim.Length > 0 Then
				...
				If myFormDataSet.Tables(0).Rows.Count = 1 Then
					myFormDAO.UpdateEntity(myFormID, myMeetingRecordID, myLawID, myCause, myManagementInstitutionID, myUndertakerID, myDocumentNumber, mySignID, myConcern, myDraftID)
				Else
					'exception:form record is empty or duplicated
				End If
			Else
				'insert new data
				myFormID = myFormDAO.InsertEntity(myMeetingRecordID, myLawID, myCause, myManagementInstitutionID, myUndertakerID, myDocumentNumber, mySignID, myConcern, myDraftID)
			End If
			Return myFormID
		End Function
		...
		Private Sub ButtonFormUpdate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonFormUpdate.Click
			If formID.Trim.Length > 0 Then
				SaveFormData(formID)
				Response.Redirect("~/DesktopModules/AuditSystem/PolicyInsuranceLawCheckForm.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&formID=" & formID & "&commentID=" & commentID & "&insuranceID=" & insuranceID)
			Else
				'exception:form id is empty
			End If
		End Sub
			...
			If formID.Trim.Length > 0 Then
				...
				If myFormDataSet.Tables(0).Rows.Count = 1 Then
					...
					If myFormDataSet.Tables(0).Rows.Count > 0 Then
						...
						Next
						If bFound = True Then
							Response.Redirect("~/DesktopModules/AuditSystem/PolicyInsuranceLawCheckForm.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&formID=" & myPreviousID & "&commentID=" & commentID & "&insuranceID=" & insuranceID)
						End If
					End If
				Else
					'exception:form record is empty or duplicated
			...
			If formID.Trim.Length > 0 Then
				...
				If myFormDataSet.Tables(0).Rows.Count = 1 Then
					...
					If bFound = True Then
						...
						End If
						Response.Redirect("~/DesktopModules/AuditSystem/PolicyInsuranceLawCheckForm.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&formID=" & myNextID & "&commentID=" & commentID & "&insuranceID=" & insuranceID)
					End If
				Else
					'exception:form record is empty or duplicated
				End If
				...
				If myFormDataSet.Tables(0).Rows.Count = 1 Then
					...
					If commentID.Trim.Length > 0 Then
						If commentAction.Trim.Length > 0 Then
							If commentAction.Trim = "update" Then
								'update
								myCommentDAO.UpdateEntity(commentID, formID, myCommentNumber, myComment, 1)
								Response.Redirect("~/DesktopModules/AuditSystem/PolicyInsuranceLawCheckForm.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&formID=" & formID & "&commentID=" & commentID & "&insuranceID=" & insuranceID)
							Else
								If commentAction.Trim = "delete" Then
					...
					Else
						'insert
						myCommentDAO.InsertEntity(formID, myCommentNumber, myComment, 1)
						Response.Redirect("~/DesktopModules/AuditSystem/PolicyInsuranceLawCheckForm.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&formID=" & formID & "&commentID=" & commentID & "&insuranceID=" & insuranceID)
					End If
				Else
					'exception:form record is empty or duplicated
		...
		Private Function SaveInsuranceMapData(ByVal myInsuranceMapID As String) As String
			...
			If formID.Trim.Length > 0 Then
				...
				If myInsuranceMapID.Trim.Length > 0 Then
					...
					If myInsuranceMapDataSet.Tables(0).Rows.Count = 1 Then
						myInsuranceMapDAO.UpdateEntity(myInsuranceMapID, formID, myInsuranceID, myProcessDate, myForecastdate, myOutsideProcessState, myInsideProcessState, myUndertakerID, myConcludeDate, myConcludeNumber)
						'update member
				...
				Else
					'insert
					myInsuranceMapID = myInsuranceMapDAO.InsertEntity(formID, myInsuranceID, myProcessDate, myForecastdate, myOutsideProcessState, myInsideProcessState, myUndertakerID, myConcludeDate, myConcludeNumber)
					'insert member
					SaveInsuranceMemberData(myInsuranceMapID)
				End If
			Else
			...
			If myInsuranceMapID.Trim.Length > 0 Then
				For i = 0 To ListBoxMember.Items.Count - 1
					...
					If myListItem.Value.Trim.Length > 0 Then
						...
						myMemberMapDAO.InsertEntity(myInsuranceMapID, myMemberID, 1)
					End If
				Next
			Else
				'exception:insurance map id is empty
		...
		Private Sub ButtonInsuranceInsert_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonInsuranceInsert.Click
			...
			If formID.Trim.Length > 0 Then
				...
				If myInsureanceMapID.Trim.Length > 0 Then
					Response.Redirect("~/DesktopModules/AuditSystem/PolicyInsuranceLawCheckForm.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&formID=" & formID & "&commentID=" & commentID & "&insuranceID=" & myInsureanceMapID)
				Else
					'insert failure
					PageLoad()
				End If
				...
				If insuranceID.Trim.Length > 0 Then
					...
					If myInsuranceMapDataSet.Tables(0).Rows.Count = 1 Then
						...
						If myInsuranceMapDataSet.Tables(0).Rows.Count > 0 Then
							...
							Next
							If bFound = True Then
								Response.Redirect("~/DesktopModules/AuditSystem/PolicyInsuranceLawCheckForm.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&formID=" & formID & "&commentID=" & commentID & "&insuranceID=" & myPreviousID)
							End If
						End If
					Else
						'exception:insurance map record is empty or duplicated
					...
					If myInsuranceMapDataSet.Tables(0).Rows.Count = 1 Then
						...
						If myInsuranceMapDataSet.Tables(0).Rows.Count > 0 Then
							...
							If bFound = True Then
								...
								End If
								Response.Redirect("~/DesktopModules/AuditSystem/PolicyInsuranceLawCheckForm.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&formID=" & formID & "&commentID=" & commentID & "&insuranceID=" & myNextID)
							End If
						End If
					Else
						'exception:insurance map record is empty or duplicated
