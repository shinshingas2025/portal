...
#End Region
		...
		Private Sub PageLoad()
			...
			Else
				If categorizationID.Trim.Length > 0 Then
					myNewsDataSet = myNewsDAO.GetEntitysByCategorizationID(categorizationID, 1)
					If myNewsDataSet.Tables(0).Rows.Count = 1 Then
						myNewsID = CType(myNewsDataSet.Tables(0).Rows(0).Item("EntityID"), String)
						Response.Redirect("~/DesktopModules/AuditSystem/NewsReleaseAdmin.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&categorizationID=" & categorizationID & "&newsID=" & myNewsID)
					End If
				Else
			...
			If myNewsID.Trim.Length > 0 Then
				...
				If myContentCount > 0 Then
					...
					For i = 0 To myContentCount - 1
						...
						myHyperLink.NavigateUrl = "~/DesktopModules/AuditSystem/NewsReleaseAdmin.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&categorizationID=" & categorizationID & "&newsID=" & myNewsID & "&contentID=" & myContentID & "&action=update"
						...
						'insert link
						myHyperLink = New HyperLink
						myHyperLink.NavigateUrl = "~/DesktopModules/AuditSystem/NewsReleaseAdmin.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&categorizationID=" & categorizationID & "&newsID=" & myNewsID & "&contentID=" & myContentID & "&action=delete"
			...
			If myNewsID.Trim.Length > 0 Then
				...
				If myAppendCount > 0 Then
					...
					For i = 0 To myAppendCount - 1
						...
						'insert link
						myHyperLink = New HyperLink
						myHyperLink.NavigateUrl = "~/DesktopModules/AuditSystem/NewsReleaseAdmin.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&categorizationID=" & categorizationID & "&newsID=" & myNewsID & "&appendID=" & myAppendID & "&action=update"
						myHyperLink.ImageUrl = "~/images/edit.gif"
						myHyperLink.Text = "修改"
						myTableCell.Controls.Add(myHyperLink)
						'br
						...
						'insert link
						myHyperLink = New HyperLink
						myHyperLink.NavigateUrl = "~/DesktopModules/AuditSystem/NewsReleaseAdmin.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&categorizationID=" & categorizationID & "&newsID=" & myNewsID & "&appendID=" & myAppendID & "&action=delete"
						myHyperLink.ImageUrl = "~/images/delete.gif"
						myHyperLink.Text = "刪除"
						myTableCell.Controls.Add(myHyperLink)
		...
		Private Function SaveNewsData(ByVal myNewsID As String) As String
			...
			If myNewsID.Trim.Length > 0 Then
				...
				If myNewsDataSet.Tables(0).Rows.Count = 1 Then
					'update news record
					myNewsDAO.UpdateEntity(myNewsID, myCategorizationID, myTitle, myDescription, myOpening, myEnding, myRelationURL, myReleaseUnitID, myLiaisonerID, myNewsDate, myStartDate, myEndDate)
				Else
					'exception:news record is empty or duplicated
				End If
			Else
				'insert new data
				myNewsID = myNewsDAO.InsertEntity(myCategorizationID, myTitle, myDescription, myOpening, myEnding, myRelationURL, myReleaseUnitID, myLiaisonerID, myNewsDate, myStartDate, myEndDate)
			End If
			Return myNewsID
		End Function
			...
			If newsID.Trim.Length > 0 Then
				...
				If myNewsDataSet.Tables(0).Rows.Count = 1 Then
					...
					If myNewsDataSet.Tables(0).Rows.Count > 0 Then
						...
						Next
						If bFound = True Then
							Response.Redirect("~/DesktopModules/AuditSystem/NewsReleaseAdmin.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&categorizationID=" & categorizationID & "&newsID=" & myPreviousID)
						End If
					End If
				Else
				...
				If myNewsDataSet.Tables(0).Rows.Count = 1 Then
					...
					If myNewsDataSet.Tables(0).Rows.Count > 0 Then
						...
						If bFound = True Then
							...
							End If
							Response.Redirect("~/DesktopModules/AuditSystem/NewsReleaseAdmin.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&categorizationID=" & categorizationID & "&newsID=" & myNextID)
						End If
					End If
				Else
					'exception:record is empty or duplicated
		...
		Private Sub ButtonNewsUpdate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonNewsUpdate.Click
			If newsID.Trim.Length > 0 Then
				SaveNewsData(newsID)
				Response.Redirect("~/DesktopModules/AuditSystem/NewsReleaseAdmin.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&categorizationID=" & categorizationID & "&newsID=" & newsID)
			Else
		...
		Private Sub ButtonNewsDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonNewsDelete.Click
			If newsID.Trim.Length > 0 Then
				...
				newsID = ""
				Response.Redirect("~/DesktopModules/AuditSystem/NewsReleaseAdmin.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&categorizationID=" & categorizationID)
			Else
				'exception:form id is empty
			End If
		End Sub
		...
		Private Sub DropDownListCategorization_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DropDownListCategorization.SelectedIndexChanged
			Dim myCategorizationID As String = ""
			myCategorizationID = DropDownListCategorization.SelectedValue
			Response.Redirect("~/DesktopModules/AuditSystem/NewsReleaseAdmin.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&categorizationID=" & myCategorizationID)
		End Sub
		Private Sub ButtonContentAction_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonContentAction.Click
			...
			If newsID.Trim.Length > 0 Then
				...
				If contentID.Trim.Length > 0 Then
					If action.Trim.Length > 0 Then
						If action.Trim = "update" Then
							...
							If myContentDataSet.Tables(0).Rows.Count = 1 Then
								'actual action
								myContentDAO.UpdateEntity(contentID, newsID, myContentNumber, myContent, myContentOrder)
								Response.Redirect("~/DesktopModules/AuditSystem/NewsReleaseAdmin.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&categorizationID=" & categorizationID & "&newsID=" & newsID)
							Else
								'exception:content record is empty or duplicated
							End If
				...
				Else
					'insert new content
					myContentDAO.InsertEntity(newsID, myContentNumber, myContent, myContentOrder)
					Response.Redirect("~/DesktopModules/AuditSystem/NewsReleaseAdmin.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&categorizationID=" & categorizationID & "&newsID=" & newsID)
				End If
			Else
				'exception:news id is empty
		...
		Private Sub ButtonAppendAction_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonAppendAction.Click
			...
			If newsID.Trim.Length > 0 Then
					...
					If action.Trim.Length > 0 Then
						If action.Trim = "update" Then
							...
							If myAppendDataSet.Tables(0).Rows.Count = 1 Then
								...
								End If
								'actual action
								myAppendDAO.UpdateEntity(appendID, newsID, myAppendName, myAppendDescription, myPhysicalFileName, myFileSize, myAppendOrder)
								Response.Redirect("~/DesktopModules/AuditSystem/NewsReleaseAdmin.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&categorizationID=" & categorizationID & "&newsID=" & newsID)
							Else
				...
				Else
					...
					End If
					'actual action
					myAppendDAO.InsertEntity(newsID, myAppendName, myAppendDescription, myPhysicalFileName, myFileSize, myAppendOrder)
					Response.Redirect("~/DesktopModules/AuditSystem/NewsReleaseAdmin.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&categorizationID=" & categorizationID & "&newsID=" & newsID)
				End If
			Else
				'exception:news id is empty
