...
Namespace ASPNET.StarterKit.Portal.AuditSystem.Module
	...
	Public Class NewsReleaseViewFrame
			...
			If newsID.Trim.Length > 0 Then
				...
				If myNewsDataSet.Tables(0).Rows.Count = 1 Then
					...
					If myNewsDataSet.Tables(0).Rows.Count > 0 Then
						...
						Next
						If bFound = True Then
							Response.Redirect("~/DesktopModules/AuditSystem/NewsReleaseViewFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&newsID=" & myPreviousID)
						End If
					End If
				Else
					'exception:news record is empty or duplicated
				...
				If myNewsDataSet.Tables(0).Rows.Count = 1 Then
					...
					If myNewsDataSet.Tables(0).Rows.Count > 0 Then
						...
						If bFound = True Then
							...
							End If
							Response.Redirect("~/DesktopModules/AuditSystem/NewsReleaseViewFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&newsID=" & myNextID)
						End If
					End If
				Else
					'exception:news record is empty or duplicated
