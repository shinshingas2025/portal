...
Namespace ASPNET.StarterKit.Portal.AuditSystem.Module
	...
	Public Class MeetingRecordAdmin
...
#Region " Web Form 設計工具產生的程式碼 "
		...
		Protected WithEvents PlaceHolderResolution As System.Web.UI.WebControls.PlaceHolder
		Protected WithEvents ButtonResolutionAction As System.Web.UI.WebControls.Button
		...
		Protected WithEvents DropDownListSketch As System.Web.UI.WebControls.DropDownList
		...
		Protected WithEvents AjaxPanelActiveResolution As MagicAjax.UI.Controls.AjaxPanel
...
#End Region
		...
		Private Const AffairColumnWidth As String = "280"
		Private Const AssistOfficeColumnWidth As String = "64"
		Private Const SketchColumnWidth As String = "180"
		...
		Private Sub PageLoad()
			...
			Else
				If typeID.Trim.Length > 0 Then
					myRecordDataSet = myRecordDAO.GetEntitysByTypeID(typeID, 1)
					If myRecordDataSet.Tables(0).Rows.Count = 1 Then
						myRecordID = CType(myRecordDataSet.Tables(0).Rows(0).Item("EntityID"), String)
						Response.Redirect("~/DesktopModules/AuditSystem/MeetingRecordAdmin.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&typeID=" & typeID & "&recordID=" & myRecordID)
					End If
				Else
				...
				If myRecordDataSet.Tables(0).Rows.Count = 1 Then
					...
					If myResolutionCount > 0 Then
						...
						For i = 0 To myResolutionCount - 1
							...
							myHyperLink.NavigateUrl = "~/DesktopModules/AuditSystem/MeetingRecordAdmin.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&typeID=" & typeID & "&recordID=" & myRecordID & "&resolutionID=" & myResolutionID & "&action=update"
							...
							'insert link
							myHyperLink = New HyperLink
							myHyperLink.NavigateUrl = "~/DesktopModules/AuditSystem/MeetingRecordAdmin.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&typeID=" & typeID & "&recordID=" & myRecordID & "&resolutionID=" & myResolutionID & "&action=delete"
		...
		Private Sub InitialWebControl()
			...
			myNormalCodeCount = myNormalCodeDAO.GetTotalRowByGroupID(PlaceCodeGroupID)
			If myNormalCodeCount > 0 Then
			...
			myNormalCodeCount = myNormalCodeDAO.GetTotalRowByGroupID(PresentCodeGroupID)
			If myNormalCodeCount > 0 Then
			...
			myNormalCodeCount = myNormalCodeDAO.GetTotalRowByGroupID(SketchCodeGroupID)
			If myNormalCodeCount > 0 Then
		...
		Private Function SaveRecordData(ByVal myRecordID As String) As String
			...
			If myRecordID.Trim.Length > 0 Then
				...
				If myRecordDataSet.Tables(0).Rows.Count = 1 Then
						...
						For i = 0 To myResolutionCount - 1
							...
							If myNumber.Length > 0 Then
								...
								If Not (myFormDataSet Is Nothing) Then
									...
									'actual action
									'myFormDAO.UpdateEntityByTypeID(myTypeID, myFormID, myAffairID, myMainOfficeID, myAssistOfficeID, myNumber, myMeetingNumber, myMeetingDate)
								Else
									'exception:form record is empty or duplicated
								End If
							Else
					...
					myRecordDAO.UpdateEntity(myRecordID, myTypeID, myMeetingNumber, myMeetingDate, myStartTime, myEndTime, myPlaceID, myPresentPerson, myObserver, myChairPersonID, myScribeID, myTitle, myPlaceName)
				Else
					'exception:meeting record is empty or duplicated
				End If
			Else
				...
				If typeID.Trim.Length > 0 Then
					If myRecordDAO.GetTotalRowByTypeIDAndMeetingNumber(typeID, myMeetingNumber) = 0 Then
						myRecordID = myRecordDAO.InsertEntity(typeID, myMeetingNumber, myMeetingDate, myStartTime, myEndTime, myPlaceID, myPresentPerson, myObserver, myChairPersonID, myScribeID, myTitle, myPlaceName)
					Else
			...
			If recordID.Trim.Length > 0 Then
				...
				If myRecordDataSet.Tables(0).Rows.Count = 1 Then
					...
					If myRecordDataSet.Tables(0).Rows.Count > 0 Then
						...
						Next
						If bFound = True Then
							Response.Redirect("~/DesktopModules/AuditSystem/MeetingRecordAdmin.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&typeID=" & typeID & "&recordID=" & myPreviousID)
						End If
					End If
				Else
					'exception:resolution record is empty or duplicated
				...
				If myRecordDataSet.Tables(0).Rows.Count = 1 Then
					...
					If myRecordDataSet.Tables(0).Rows.Count > 0 Then
						...
						If bFound = True Then
							...
							End If
							Response.Redirect("~/DesktopModules/AuditSystem/MeetingRecordAdmin.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&typeID=" & typeID & "&recordID=" & myNextID)
						End If
					End If
				Else
					'exception:resolution record is empty or duplicated
		...
		Private Sub ButtonRecordUpdate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonRecordUpdate.Click
			If recordID.Trim.Length > 0 Then
				...
				Response.Redirect("~/DesktopModules/AuditSystem/MeetingRecordAdmin.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&typeID=" & typeID & "&recordID=" & recordID)
			Else
		...
		Private Sub ButtonRecordDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonRecordDelete.Click
			If recordID.Trim.Length > 0 Then
				...
				recordID = ""
				Response.Redirect("~/DesktopModules/AuditSystem/MeetingRecordAdmin.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&typeID=" & typeID)
			Else
				'exception:form id is empty
			End If
		End Sub
		...
		Private Sub DropDownListType_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DropDownListType.SelectedIndexChanged
			Dim myTypeID As String = ""
			myTypeID = DropDownListType.SelectedValue
			Response.Redirect("~/DesktopModules/AuditSystem/MeetingRecordAdmin.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&typeID=" & myTypeID)
		End Sub

		Private Sub ButtonResolutionAction_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonResolutionAction.Click
			Dim myResolutionDAO As New MeetingRecordResolutionDAOExtand
			Dim myResolutionDataSet As DataSet
			Dim myRecordDAO As New MeetingRecordDAOExtand
			Dim myRecordDataSet As DataSet
				...
				If myRecordDataSet.Tables(0).Rows.Count = 1 Then
							...
							If action.Trim = "update" Then
								...
								If myResolutionDataSet.Tables(0).Rows.Count = 1 Then
									...
									If Not (myFormDataSet Is Nothing) Then
										...
										'myFormDAO.UpdateEntityByTypeID(myTypeID, myFormID, myAffairID, myMainOfficeID, myAssistOfficeID, myNumber, myMeetingNumber, myMeetingDate)
										'update resolution
										myResolutionDAO.UpdateEntity(resolutionID, recordID, myNumber, myContent, myMainOfficeID, myAssistOfficeID, mySketchID)
										Response.Redirect("~/DesktopModules/AuditSystem/MeetingRecordAdmin.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&typeID=" & typeID & "&recordID=" & recordID)
									Else
					...
					Else
						...
						If myNumber.Trim.Length > 0 Then
							...
							'myFormDAO.InsertEntityByTypeID(myTypeID, myAffairID, myMainOfficeID, myAssistOfficeID, myNumber, myMeetingNumber, myMeetingDate)
							'insert resolution
							myResolutionID = myResolutionDAO.InsertEntity(recordID, myNumber, myContent, myMainOfficeID, myAssistOfficeID, mySketchID)
							Response.Redirect("~/DesktopModules/AuditSystem/MeetingRecordAdmin.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&typeID=" & typeID & "&recordID=" & recordID)
						Else
							'exception:resolution number is empty
						End If
		...
		Private Function GetResolutionNumber() As String
			...
			Return result
		End Function
		Protected Sub DropDownListMainOffice_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DropDownListMainOffice.SelectedIndexChanged
			Dim myNormalCodeDAO As New NormalCodeDAOExtand
			Dim myNormalCodeDataSet As DataSet
			Dim myNormalCodeCount As Integer = 0
			Dim i As Integer = 0
