...
Namespace ASPNET.StarterKit.Portal.AuditSystem.Module
	...
	Public Class AttributeCodeAdminFrame
...
#End Region
		...
		Private Const CodeAuthorityTarget As String = "ProcessAttributeCode"
		...
		Private Sub PageLoad()
			Dim myCodeDAO As New ProcessAttributeCodeDAOExtand
		...
		Private Sub DropDownListCode_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DropDownListCode.SelectedIndexChanged
			Response.Redirect("~/DesktopModules/AuditSystem/AttributeCodeAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&codeID=" & DropDownListCode.SelectedValue)
		End Sub
		...
		Private Function SaveCodeData(ByVal myCodeID As String) As String
			...
			If myCodeID.Trim.Length > 0 Then
				'get authority
				If myAuthorityBO.CheckPurview(CodeAuthorityTarget, myCodeID, Context.User.Identity.Name, "U") Then
					'update
					myCodeDAO.UpdateEntity(myCodeID, myName, myDescription, myDisplayOrder, Context.User.Identity.Name, Now)
				End If
			Else
				'insert
				If myName.Trim.Length > 0 Then
					...
					If myCodeCount = 0 Then
						myCodeID = myCodeDAO.InsertEntity(0, AttributeCodeType.Checkbox, myName, myDescription, myDisplayOrder, DefaultPermissionGroup, DefaultPermission, Context.User.Identity.Name, Now, Context.User.Identity.Name, Now, 1, New Date(1900, 1, 1))
					Else
		...
		Private Sub ButtonInsert_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonInsert.Click
			...
			If myCodeID.Trim.Length > 0 Then
				Response.Redirect("~/DesktopModules/AuditSystem/AttributeCodeAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&codeID=" & myCodeID)
			Else
