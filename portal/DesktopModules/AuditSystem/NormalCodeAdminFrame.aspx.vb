...
Namespace ASPNET.StarterKit.Portal.AuditSystem.Module
	...
	Public Class NormalCodeAdminFrame
...
#End Region
		...
		Private Const CodeGroupAuthorityTarget As String = "NormalCodeGroup"
		...
		Private Const NameColumnWidth As String = "200"
		Private Const DescriptionColumnWidth As String = "300"
		...
		Private Sub PageLoad()
			...
			myCodeGroupDataSet = UtilityObject.QueryPermissionFilter(myCodeGroupDataSet, CodeGroupAuthorityTarget, Context.User.Identity.Name)
		...
		Private Sub FillCodeData(ByVal myCodeGroupID As String)
			...
			If myCodeGroupID.Trim.Length > 0 Then
				...
				If myCodeCount > 0 Then
					...
					myCodeDataSet = UtilityObject.QueryPermissionFilter(myCodeDataSet, CodeAuthorityTarget, Context.User.Identity.Name)

					If myCodeDataSet.Tables(0).Rows.Count > 0 Then
						For i = 0 To myCodeDataSet.Tables(0).Rows.Count - 1
							...
							myHyperLink.NavigateUrl = "~/DesktopModules/AuditSystem/NormalCodeAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&codeGroupID=" & codeGroupID & "&codeID=" & myCodeID & "&action=update"
							...
							'insert link
							myHyperLink = New HyperLink
							myHyperLink.NavigateUrl = "~/DesktopModules/AuditSystem/NormalCodeAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&codeGroupID=" & codeGroupID & "&codeID=" & myCodeID & "&action=delete"
		...
		Private Sub FillCodeGroupData(ByVal myDataSet As DataSet)
			...
			If (Not (myDataSet Is Nothing)) Then
				...
				Else
					'exception:code group record is empty
				End If
			Else
		...
		Private Sub InitialWebControl()
			...
			If myCodeGroupCount > 0 Then
				...
				If myCodeGroupDataSet.Tables(0).Rows.Count > 0 Then
					For i = 0 To myCodeGroupDataSet.Tables(0).Rows.Count - 1
		...
		Private Sub DropDownListCodeGroup_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DropDownListCodeGroup.SelectedIndexChanged
			Response.Redirect("~/DesktopModules/AuditSystem/NormalCodeAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&codeGroupID=" & DropDownListCodeGroup.SelectedValue)
		End Sub
		...
		Private Function SaveCodeGroupData(ByVal myCodeGroupID As String) As String
			...
			If myCodeGroupID.Trim.Length > 0 Then
				'get authority
				If myAuthorityBO.CheckPurview(CodeGroupAuthorityTarget, myCodeGroupID, Context.User.Identity.Name, "U") Then
					'update
					myCodeGroupDAO.UpdateEntity(myCodeGroupID, myGroupName, myGroupDescription, myGroupDisplayOrder, Context.User.Identity.Name, Now)
				End If
			Else
				'insert
				If myGroupName.Trim.Length > 0 Then
					...
					If myCodeGroupCount = 0 Then
						myCodeGroupID = myCodeGroupDAO.InsertEntity(0, myGroupName, myGroupDescription, myGroupDisplayOrder, DefaultPermissionGroup, DefaultPermission, Context.User.Identity.Name, Now, Context.User.Identity.Name, Now, 1, New Date(1900, 1, 1))
					Else
		...
		Private Sub ButtonGroupInsert_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonGroupInsert.Click
			...
			If myCodeGroupID.Trim.Length > 0 Then
				Response.Redirect("~/DesktopModules/AuditSystem/NormalCodeAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&codeGroupID=" & myCodeGroupID)
			Else
		...
		Private Sub ButtonCodeAction_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonCodeAction.Click
			...
			If codeGroupID.Trim.Length > 0 Then
				...
				If codeID.Trim.Length > 0 Then
					If action.Trim = "update" Then
						'get authority
						If myAuthorityBO.CheckPurview(CodeAuthorityTarget, codeID, Context.User.Identity.Name, "U") Then
							'update
							myCodeDAO.UpdateEntity(codeID, myCodeName, myCodeDescription, myCodeDisplayOrder, Context.User.Identity.Name, Now)
							Response.Redirect("~/DesktopModules/AuditSystem/NormalCodeAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&codeGroupID=" & codeGroupID)
						Else
							PageLoad()
						End If
					Else
						If action.Trim = "delete" Then
							'get authority
							If myAuthorityBO.CheckPurview(CodeAuthorityTarget, codeID, Context.User.Identity.Name, "D") Then
								'delete
								myCodeDAO.UpdateEntity(codeID, CodeState.Disable, Now, Context.User.Identity.Name, Now)
								Response.Redirect("~/DesktopModules/AuditSystem/NormalCodeAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&codeGroupID=" & codeGroupID)
							Else
								PageLoad()
							End If
				...
				Else
					'insert
					myCodeDAO.InsertEntity(codeGroupID, 0, myCodeName, myCodeDescription, myCodeDisplayOrder, DefaultPermissionGroup, DefaultPermission, Context.User.Identity.Name, Now, Context.User.Identity.Name, Now, 1, New Date(1900, 1, 1))
					Response.Redirect("~/DesktopModules/AuditSystem/NormalCodeAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&codeGroupID=" & codeGroupID)
				End If
			Else
				'exception:code group id is empty
