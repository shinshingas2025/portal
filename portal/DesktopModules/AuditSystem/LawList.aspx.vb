...
#End Region
		...
		Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
			...
			If Not IsPostBack Then
				...
				rowCount = myLawDataSet.Tables(0).Rows.Count
		...
		Private Sub PageLoad()
			...
			Dim myLawDAO As New LawDAOExtand
			Dim myLawDataSet As DataSet
			Dim myParentLawDataSet As DataSet
			...
			myLawDataSet = myLawDAO.GetEntitys()
			myLawDataSet = UtilityObject.QueryPermissionFilter(myLawDataSet, LawAuthorityTarget, Context.User.Identity.Name, pageSize * currentEntityListPage)

			myDataColumn = New DataColumn("ParentName")
			myLawDataSet.Tables(0).Columns.Add(myDataColumn)
			myDataColumn = New DataColumn("ConstitutionDateString")
			myLawDataSet.Tables(0).Columns.Add(myDataColumn)
			...
			If myLawDataSet.Tables(0).Rows.Count > 0 Then
				...
				If currentEntityListPage > 1 Then
					For i = 0 To currentEntityListPage - 2
						For j = 0 To pageSize - 1
							myLawDataSet.Tables(0).Rows(i * pageSize + j).Delete()
						Next
					Next
				End If
				...
				For i = lowBound To highBound
					...
					Else
						...
						myHyperLink.NavigateUrl = "~/DesktopModules/AuditSystem/LawList.aspx?ReturnObjectID=" & returnObjectID & "&CurrentEntityListPage=" & CType(i, String)
				...
				'prepare law data
				If myLawDataSet.Tables(0).Rows.Count > 0 Then
					For i = (currentEntityListPage - 1) * pageSize To myLawDataSet.Tables(0).Rows.Count - 1
						'parent name
						myParentID = CType(myLawDataSet.Tables(0).Rows(i).Item("ParentID"), String).Trim
						myParentName = ""
						If myParentID.Length > 0 Then
							myParentLawDataSet = myLawDAO.GetEntitysByEntityID(myParentID)
							If myParentLawDataSet.Tables(0).Rows.Count = 1 Then
								myParentName = CType(myParentLawDataSet.Tables(0).Rows(0).Item("Name"), String).Trim
								myLawDataSet.Tables(0).Rows(i).Item("ParentName") = myParentName
							Else
								'exception:law record is empty or duplicated
							End If
						End If
						'constitution date string
						myConstitutionDate = CType(myLawDataSet.Tables(0).Rows(i).Item("ConstitutionDate"), Date)
						If myConstitutionDate = myEmptyDate Then
							myLawDataSet.Tables(0).Rows(i).Item("ConstitutionDateString") = ""
						Else
							myLawDataSet.Tables(0).Rows(i).Item("ConstitutionDateString") = myConstitutionDate.Year & "/" & myConstitutionDate.Month & "/" & myConstitutionDate.Day
						End If
					Next
				End If
			Else
				'exception
			End If
			DataList1.DataSource = myLawDataSet
			DataList1.DataBind()
		End Sub
		Sub PageReload()
			Dim myLawDAO As New LawDAOExtand
		...
		Private Sub LinkButtonEntityListPageDown_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles LinkButtonEntityListPageDown.Click
			currentEntityListPage = currentEntityListPage + 1
			If currentEntityListPage > totalEntityListPage Then
		...
		Private Sub LinkButtonEntityListTenPageDown_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles LinkButtonEntityListTenPageDown.Click
			currentEntityListPage = currentEntityListPage + 10
			If currentEntityListPage > totalEntityListPage Then
				currentEntityListPage = totalEntityListPage
			End If
