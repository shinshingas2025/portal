...
Namespace ASPNET.StarterKit.Portal.AuditSystem.Module
	...
	Public Class CouncilmanInstructionAdminFrame
...
#End Region
		...
		Private Sub PageLoad()
			...
			Else
				...
				myInstructionDataSet = UtilityObject.QueryPermissionFilter(myInstructionDataSet, InstructionAuthorityTarget, Context.User.Identity.Name, 1)
				...
				If myInstructionDataSet.Tables(0).Rows.Count = 1 Then
					...
					Response.Redirect("~/DesktopModules/AuditSystem/CouncilmanInstructionAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&instructionID=" & myInstructionID)
				End If
			End If
		...
		Private Sub FillCouncilmanData(ByVal myInstructionID As String)
			...
			If myInstructionID.Trim.Length > 0 Then
				...
				If myCouncilmanCount > 0 Then
					...
					myCouncilmanDataSet = UtilityObject.QueryPermissionFilter(myCouncilmanDataSet, InstructionMemberAuthorityTarget, Context.User.Identity.Name)
					myCouncilmanCount = myCouncilmanDataSet.Tables(0).Rows.Count
					...
					If myCouncilmanCount > 0 Then
						For i = 0 To myCouncilmanCount - 1
							...
							myHyperLink.NavigateUrl = "~/DesktopModules/AuditSystem/CouncilmanInstructionAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&instructionID=" & myInstructionID & "&mapID=" & myMapID & "&action=update"
							...
							'insert link
							myHyperLink = New HyperLink
							myHyperLink.NavigateUrl = "~/DesktopModules/AuditSystem/CouncilmanInstructionAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&instructionID=" & myInstructionID & "&mapID=" & myMapID & "&action=delete"
		...
		Private Function SaveInstructionData(ByVal myInstructionID As String) As String
			...
			If myInstructionID.Trim.Length > 0 Then
				...
				If myInstructionDataSet.Tables(0).Rows.Count = 1 Then
					'get authority
					If myAuthorityBO.CheckPurview(InstructionAuthorityTarget, myInstructionID, Context.User.Identity.Name, "U") Then
						'update instruction record
						myInstructionDAO.UpdateEntity(myInstructionID, myTitle, myDescription, myInstruction, myNote, myInstructionDate, Context.User.Identity.Name, Now)
					End If
				Else
			...
			Else
				'insert new data
				myInstructionID = myInstructionDAO.InsertEntity(0, myTitle, myDescription, myInstruction, myNote, myInstructionDate, Context.User.Identity.Name, Now, Context.User.Identity.Name, Now, DefaultPermission, DefaultPermissionGroup, 1, New Date(1900, 1, 1))
			End If
			...
			If instructionID.Trim.Length > 0 Then
				...
				If myInstructionDataSet.Tables(0).Rows.Count = 1 Then
					...
					If myInstructionDataSet.Tables(0).Rows.Count > 0 Then
						...
						Next
						If bFound = True Then
							Response.Redirect("~/DesktopModules/AuditSystem/CouncilmanInstructionAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&instructionID=" & myPreviousID)
						End If
					End If
				Else
				...
				If myInstructionDataSet.Tables(0).Rows.Count = 1 Then
					...
					If myInstructionDataSet.Tables(0).Rows.Count > 0 Then
						...
						If bFound = True Then
							...
							End If
							Response.Redirect("~/DesktopModules/AuditSystem/CouncilmanInstructionAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&instructionID=" & myNextID)
						End If
					End If
				Else
					'exception:record is empty or duplicated
		...
		Private Sub ButtonInstructionUpdate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonInstructionUpdate.Click
			If instructionID.Trim.Length > 0 Then
				...
				Response.Redirect("~/DesktopModules/AuditSystem/CouncilmanInstructionAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&instructionID=" & instructionID)
			Else
			...
			If myInstructionID.Trim.Length > 0 Then
				...
				If myCouncilmanCount > 0 Then
					...
					For i = 0 To myCouncilmanCount - 1
						myMapID = CType(myCouncilmanDataSet.Tables(0).Rows(i).Item("EntityID"), String)
						'get authority
						If myAuthorityBO.CheckPurview(InstructionMemberAuthorityTarget, myMapID, Context.User.Identity.Name, "D") Then
		...
		Private Sub DeleteInstructionData(ByVal myInstructionID As String)
			...
			If myInstructionID.Trim.Length > 0 Then
				...
				If myInstructionDataSet.Tables(0).Rows.Count = 1 Then
					...
					'get authority
					If myAuthorityBO.CheckPurview(InstructionAuthorityTarget, myInstructionID, Context.User.Identity.Name, "D") Then
						'delete instruction
		...
		Private Sub ButtonInstructionDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonInstructionDelete.Click
			If instructionID.Trim.Length > 0 Then
				...
				instructionID = ""
				Response.Redirect("~/DesktopModules/AuditSystem/CouncilmanInstructionAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex)
			Else
				'exception:instruction id is empty
			End If
		End Sub
		...
		Private Sub ButtonCouncilmanAction_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonCouncilmanAction.Click
			...
			If instructionID.Trim.Length > 0 Then
				...
				If mapID.Trim.Length > 0 Then
					If action.Trim.Length > 0 Then
						If action.Trim = "update" Then
							...
							If myCouncilmanDataSet.Tables(0).Rows.Count = 1 Then
								'get authority
								If myAuthorityBO.CheckPurview(InstructionMemberAuthorityTarget, mapID, Context.User.Identity.Name, "U") Then
									'actual action
									myCouncilmanDAO.UpdateEntity(mapID, instructionID, 1, myCouncilmanID, Context.User.Identity.Name, Now)
									Response.Redirect("~/DesktopModules/AuditSystem/CouncilmanInstructionAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&instructionID=" & instructionID)
								Else
									PageLoad()
								End If
						...
						Else
							If action.Trim = "delete" Then
								...
								If myCouncilmanDataSet.Tables(0).Rows.Count = 1 Then
									'get authority
									If myAuthorityBO.CheckPurview(InstructionMemberAuthorityTarget, mapID, Context.User.Identity.Name, "D") Then
										'actual action
				...
				Else
					...
					myCouncilmanDAO.InsertEntity(instructionID, 0, 1, myCouncilmanID, Context.User.Identity.Name, Now, Context.User.Identity.Name, Now, DefaultPermission, DefaultPermissionGroup, 1, New Date(1900, 1, 1))
					Response.Redirect("~/DesktopModules/AuditSystem/CouncilmanInstructionAdminFrame.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&instructionID=" & instructionID)
				End If
			Else
				'exception:news id is empty
