...
Namespace ASPNET.StarterKit.Portal.AuditSystem.Module
	...
	Public Class ProcessAttributeCheckForm
...
#End Region
		...
		Private Sub PageLoad()
			...
			Else
				If formID.Trim.Length > 0 Then
					myProcessDataSet = myProcessDAO.GetEntitysByFormID(formID, 1)
		...
		Private Function SaveProcessData(ByVal myProcessID As String) As String
			...
			If formID.Trim.Length > 0 Then
				If myProcessID.Trim.Length > 0 Then
					'update
					myProcessDAO.UpdateEntity(myProcessID, myProcessDate, myProcessState, myNote)
				Else
					'insert
					myProcessID = myProcessDAO.InsertEntity(formID, myProcessDate, myProcessState, myNote)
				End If
			Else
			...
			If formID.Trim.Length > 0 Then
				If processID.Trim.Length > 0 Then
					...
					If myProcessDataSet.Tables(0).Rows.Count = 1 Then
						...
						If myProcessDataSet.Tables(0).Rows.Count > 0 Then
							...
							Next
							If bFound = True Then
								Response.Redirect("~/DesktopModules/AuditSystem/ProcessAttributeCheckForm.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&formID=" & formID & "&processID=" & myPreviousID)
							End If
						End If
					Else
				...
				Else
					...
					If myProcessDataSet.Tables(0).Rows.Count = 1 Then
						myProcessID = CType(myProcessDataSet.Tables(0).Rows(0).Item("EntityID"), String)
						If myProcessID.Trim.Length > 0 Then
							Response.Redirect("~/DesktopModules/AuditSystem/ProcessAttributeCheckForm.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&formID=" & formID & "&processID=" & myProcessID)
						Else
							'exception:process id is empty
						End If
					Else
					...
					If myProcessDataSet.Tables(0).Rows.Count = 1 Then
						...
						If myProcessDataSet.Tables(0).Rows.Count > 0 Then
							...
							If bFound = True Then
								...
								End If
								Response.Redirect("~/DesktopModules/AuditSystem/ProcessAttributeCheckForm.aspx?sid=" & sid & "&mid=" & moduleId & "&tabid=" & tabId & "&tabindex=" & tabIndex & "&formID=" & formID & "&processID=" & myNextID)
							End If
						End If
					Else
						'exception:form record is empty or duplicated
